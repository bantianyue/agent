# article_data_build.py — 由 write-article-data.py 序列化为 article_data.json
# 图的 src/caption/位置全部由 blocks.jsonl 派生（write-article-data.py 自动注入 fig_after），此处只写正文与图注翻译。

DATA = {
    "summary": [
        {"key": "精选数据≠经验", "body": "精选数据集由人类保证“数据都有用”，而真实经验里混着可学与不可学的信号，算法必须自己分辨。"},
        {"key": "SGD的盲区", "body": "SGD把所有预测误差的信用平均分配给所有非零梯度参数，连噪声也一起学进去，无法区分可预测与不可预测的目标。"},
        {"key": "IDBD会分配信用", "body": "Incremental Delta Bar Delta（IDBD）能选择性地给不同信号分配不同信用，只学数据流中可预测的部分。"},
        {"key": "NetworkIDBD", "body": "把IDBD扩展到神经网络，在NoisyMNIST上只强化中心28×28有用区域，而SGD放大了全部输入的权重。"},
        {"key": "在线学习的命门", "body": "当前靠大批量IID平均来压噪声，本质是离线思路，不适合在线持续学习；出路在“学会分配信用”的算法。"},
    ],
    "lead": [
    ...
    "conclusion": [
        "这篇博客的价值，不在于提出了一个多复杂的网络，而在于用一组“小学生也能看懂”的极简实验，把深度学习一个结构性短板钉在了墙上：SGD一视同仁地给所有误差分配信用，于是噪声也被当成知识学进了权重。",
        "大批量IID采样其实是“绕过”而非“解决”这个问题，它假设你能反复从干净分布里抽样，这本身就不是在线经验。真正要解决持续学习，方向是让算法自己学会“哪些信号值得信任”，IDBD/NetworkIDBD正是这类“学会分配信用”思路的早期样本。",
        "对做训练系统的人来说，一个很实在的启示是：当你发现模型在噪声数据上越训越歪，问题可能不在数据量，而在优化器缺乏“选择性信用分配”的能力。",
    ],
    "reference_url": "https://oaklab.ai/posts/learning-from-experience-instead-of-curated-datasets",
    # 图注逐字翻译层：键为 blocks.jsonl 文档顺序 fig_id，值为中文译文。
    # 原文无 caption 的图（fig01 仅作配图）不必出现。
    "caption_translations": {
        "fig02": "图1. 特征值与真实标签随时间的变化。标签的取值与特征相同。",
        "fig03": "图1（续）. 标签中可预测的成分。",
        "fig04": "图2. 输入与标签数据流，每步以1%概率加入±1的标签噪声。只有特征为1时出现的标签才是可预测的。",
        "fig05": "图2（续）. 作为对比的无噪声输入数据流。",
        "fig06": "图3. 带高斯噪声的标签（零均值，方差5）。可预测的成分已无法凭肉眼分辨。",
        "fig07": "图4. 在4096个特征和带噪声标签上用SGD训练的线性模型预测结果。SGD把噪声吸收进了权重参数，而非只学可预测的成分。",
        "fig08": "图5. 同一线性预测问题上IDBD的预测结果。IDBD只学到了数据流中可预测的部分。",
        "fig09": "图6. NoisyMNIST数据流中的一些样本。",
        "fig10": "图6（续）. NoisyMNIST输入样本（64×64特征）的3D视图。",
        "fig11": "图7. SGD在NoisyMNIST上学习到的网络参数。SGD增大了所有输入特征到隐藏层的输出权重，从而把输入中的噪声传递到了隐藏层特征。",
        "fig12": "图8. NetworkIDBD在NoisyMNIST上学习到的网络参数。NetworkIDBD只增大了来自中心28×28区域的权重，连接到隐藏层特征。",
    },
}
