...
DATA["lead"] = [
    ...
    "本文会沿一条清晰的因果链讲下去：先看单一模型让账单膨胀的真实案例（Uber），再看 DIY 路由的四类翻车姿势，然后走入把路由做成基础设施（Plano/两阶段）的做法，最终落到 agent 必须靠*会话固定*（session pinning）守住前缀缓存红利这个关键结论。文章所有图片与代码块均按原文完整保留。",
]
DATA["sections"] = [
  ...
  {"type":"h3","title":"今天的生产主力：Plano-Orochestrator","paras":[
