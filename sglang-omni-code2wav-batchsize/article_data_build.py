...
DATA = {
  "summary": [
    {
      "key": "同一参数两种结论",
      "body": "同样的 FIFO 配置，在 A800 上像在退步，在 H200 上却相对 batch size = 1 提升约 37.9%。"
    },
    {
      "key": "先控制再下结论",
      "body": "样本扩到 200 条、每次剔除 50 条预热、固定 CPU 核区并做争用检测，才能讨论百分之几的差异。"
    },
    {
      "key": "两次回退的收获",
      "body": "reference-aware 回退约 12%，early-emit 回退约 17.5%，反而解释清了调度成本与反馈关系。"
    }
  ],
  "lead": [
    "语音模型的 serving 性能，很容易被 host 上的噪声左右。这篇文章记录了一次围绕 Code2Wav batch size 的复测：为什么同一个参数，社区同学在 A800 上看到退步，而在 H200 上却测出近四成提升。",
    "真正有价值的不是那个最终参数，而是把一个看起来很小的配置问题，拆成可以逐项验证的控制变量实验的全过程。"
  ],
  "sections": [
    {
      "type": "h2",
      "title": "一场由 batch size 引发的争论",
      "paras": [
        "在《重新审视 CPU 资源作为语音模型 Serving 过程的一等公民》一文中，我们讨论过一个颇为尴尬的问题：同一个 commit 的吞吐可以随着主机争用大幅变化，如果连测试环境都没有控制好，所谓的优化可能只是恰好赶上了机器比较空闲。看上去是 CPU 资源的争用问题，其实更本质的思考在于，如何为我们的性能验证提供一个严谨的单一变量实验环境。同一个 commit 的性能可能受到 CPU 可用资源和争用程度这些难以察觉的变量的影响，更何况是各类模型 serving 的参数这些显性的变量呢？",
        ...
        "这组结果来自单张 H200、SeedTTS 完整 EN 数据源的前 200 条请求、并发 16、零主动等待，每个 mixed-reference 配置重复三次；每次启动服务后另做 50 条预热并剔除，CPU 绑定与争用检测条件也保持一致，完整实验记录可参考此处。",
        ...
        "<span style=\"color:#0F4C81;font-weight:bold;\">1</span>&nbsp;区分并发数、调度 batch 与模型内部的计算 batch，理解这次争论从何而来。",
        "<span style=\"color:#0F4C81;font-weight:bold;\">2</span>&nbsp;通过三组实验，分别验证收集策略、batch 上限和计算结果的返回时机对性能的影响。",
    ...
    },
    {
      ...
      "paras": [
        ...
        "在 SGLang Omni 的 benchmarking 系统中，我们需要区分客户端并发数与不同层次的 batch size：",
    ...
    {
      "type": "h2",
      "title": "MiniCPM-o 的 Code2Wav Batching 策略",
      "paras": [
        "上述讨论可以结合 MiniCPM-o 的 batching 策略来进一步理解。本文测试的是 MiniCPM-o 4.5 的语音生成路径，它属于我们在框架设计文章中讨论的 multi-stage 场景：从输入理解、内容生成，到音频 token 生成和波形还原，一条请求要经过计算特性不同的阶段；这次调整集中在末端的 Code2Wav stage，其他 stage 并未进行任何修改。",
        ...
        "最初，PR #2254 为此前逐条执行的 Code2Wav 补上了 batching 能力：调度器可以收集多条请求，模型路径也能处理多条 token 序列。不同长度的输入需要 padding，并保留有效长度来处理后续计算和输出裁剪。此外，注意到 HiFT 的实现还会按有效 mel 长度再次分组。因此，即便共享 reference，Flow 与 HiFT 实际执行的 batch 也可能不同。",
        ...
        "在 PR 2254 之前，这条 Code2Wav 路径尚不支持多请求 batching。PR 2254 早期的 commit 引入了 batching，但是却设置默认参数为 Batch size = 1，推荐用户自行开启 batch size = 4 与 10 ms 等待。我在 PR #2264 中承接这项工作，整理实现、补充测试，并将 batching 默认开启。这是一个我经常强调的原则：",
        "https://github.com/sgl-project/sglang-omni/pull/2264",
        "<span style=\"display:block;background:#f5f8fb;border-left:3px solid #0F4C81;padding:10px 14px;border-radius:4px;\">如果一个 feature 已经在明确的目标场景下验证了正确性和性能收益，就应该把它默认开启，让用户可以无感知地享受收益。开发者需要对自己的实现负责，不能只声称带来了收益，却把验证责任留给用户；无论代码是否借助 AI 完成，都应该说明适用条件，完成必要的回归验证，并清理已被替代、没有保留价值的旧 code path。如果收益尚未确认，也欢迎提交探索性的 PR，但应当公开实验条件与不确定性，不能把待验证的想法当作已经成立的优化。</span>"
      ]
    },
    {
      "type": "h3",
      "title": "默认开启，以及那几个关键参数",
      "paras": [
        ...
        "__CODE__python::def code2wav_stage(*, gpu: int, process: str) -> StageConfig:\n    return StageConfig(\n        name=\"code2wav\",\n        process=process,\n        factory_path=f\"{PKG}.stages.create_code2wav_executor\",\n        factory=FactoryArgs(\n            max_batch_size=8,\n            max_batch_wait_ms=0.0,\n            batch_wait_when_idle=False,\n        ),\n        # Note (Chenyang): As a general comment and my usual understanding\n        # of SGLang Omni, SGLang Omni has a poor runtime which leads to a\n        # underutilized GPU/SMs. To address this, we recommend users to set\n        # batchs for your compute but never wait for grouping the batchs.\n        # As SGLang Omni Runtime moves better, we shall probably wait several\n        # ms for grouping the batchs, but right now, set it to 0.0.\n        gpu=gpu,\n        terminal=True,\n    )",
        "max_batch_size 允许一次收集最多 8 条，max_batch_wait_ms 则将主动等待预算设为零；batch_wait_when_idle 约束空闲时的等待行为，在当前零预算下不会额外引入等待。除开 max_batch_size 我和 PR 2254 的作者选择略有区别外，其实 max_batch_wait_ms = 0.0 才是最主要的区别。这个选择的灵感其实源自于 PR 2202 的讨论：",
        "https://github.com/sgl-project/sglang-omni/pull/2202",
        "<span style=\"display:block;background:#f5f8fb;border-left:3px solid #0F4C81;padding:10px 14px;border-radius:4px;\">我的考虑是，如果为了组 batch 额外等待，希望这段时间 GPU 上仍有其他工作可做，让收集请求与计算尽量 overlap。对于 TTS 这种高频短请求，如果 GPU 已经没有可执行任务，CPU 却还在等下一条请求来凑 batch，主动等待就可能推迟关键路径。因此，在当时的 MiniCPM-o 实现上，我优先将等待预算设为零；是否应当增加等待，要看它能否换来足够的合批收益，而不能仅凭 runtime 有所改善就下结论。</span>",
        "作为另一个配置实例，Fun-CosyVoice3 的 vocoder 收集配置使用了 30 ms 等待预算，收集后的请求再参与 Flow 合批。它与本文 MiniCPM-o Code2Wav 的零等待配置服务于不同计算路径，不能直接据此推导哪个值更好，等待预算仍应结合各自的端到端实验评估。",
    ...
    {
      "type": "h3",
      "title": "vocoder 内部的再次分组",
      "paras": [
        "回到 Code2Wav stage 的 batching 实现，正如前文所述，虽然 Scheduler 按照 FIFO 策略，在 max batch size = 8 的情况下收集请求，但是内部的 vocoder 实际不是按照上游传入的 batch 进行计算，而是要按照 reference audio 在 batch 内再次进行分组，vocode_code2wav_payloads 中的关键循环如下：",
    ...
    },
    {
      ...
      "paras": [
        ...
        "<span style=\"color:#0F4C81;font-weight:bold;\">1</span>&nbsp;PR 2271 的测试 samples 只有 50 个，而我们发送请求的 client 端的 concurrency 就是 16，这样的测试数据量可能会引发显著的测试误差；",
        "<span style=\"color:#0F4C81;font-weight:bold;\">2</span>&nbsp;如前文所述，TTS 这种高频短请求，对 CPU core 数目是高度敏感的，而且不好意思的是，可能并不是每个开发者都知道这个事情，因此我需要在重新验证的时候引入对 CPU core 数目这一变量的控制，参考 重新审视 CPU 资源作为语音模型 Serving 过程的一等公民一文中的 CPU 资源争用问题；",
        "https://github.com/zhaochenyang20/Awesome-ML-SYS-Tutorial/blob/main/sglang/sglang-omni/cpu-first-class-citizen-zh.md",
        "<span style=\"color:#0F4C81;font-weight:bold;\">3</span>&nbsp;作者的实验同时变动了两个参数：一个是是否开启 reference-aware batching，另一个是 max batch size。我们应当分别控制变量来验证这两个参数的独立效应；"
      ]
    },
    {
      "type": "h3",
      "title": "样本、预热与 CPU 共同构成实验条件",
      "paras": [
    ...
    },
    {
      ...
      "paras": [
        ...
        "这些调用数沿用公开记录的服务日志口径，用来观察配置之间的机制差异；它们不是 GPU kernel 数，也不应直接当成恰好覆盖正式 200 条请求的计数来反推平均 batch。",
        "https://github.com/sgl-project/sglang-omni/pull/2271#issuecomment-5751596133",
        ...
        "为了进一步检验 reference 混合程度的影响，我们用 --no-ref-audio 做了一个共享默认 reference 的对照。batch size = 8 下，FIFO 与 reference-aware 分别为 7.122 和 7.069 requests/s，只差约 0.7%；去掉 reference 差异后，两种策略趋于接近，支持前面的机制解释。不过这个对照也改变了输入与参考条件处理，重复次数有限，不能仅凭它排除所有其他影响，具体结果可以参考对照结果。",
        "https://github.com/sgl-project/sglang-omni/pull/2271#issuecomment-5751596133"
      ],
      "fig_after": {
        "0": [
    ...
    },
    {
      ...
      "paras": [
        ...
        "吞吐单位均为 requests/s。**本表每个配置仅运行一轮干净实验，用于观察趋势；完整 EN 的前 50 条与官方 SeedTTS-50 不是同一份 workload。** 独立实验记录可参考此处。",
    ...
    },
    {
      ...
      "table": {
        ...
        "rows": [
          [
            "Wait-all，整批完成后返回",
            "<strong style=\"color:#1a7f5a;\">5.730</strong>",
            "<strong style=\"color:#1a7f5a;\">2.739</strong>",
            "4.96",
            "约 74%"
          ],
          [
    ...
    },
    {
      ...
      "title": "提前返回后发生了什么",
      "paras": [
        "提前返回后，吞吐下降约 17.5%，平均延迟增加约 20.1%，实际收集的 batch size 仍然约为 7，reference group 数也基本一致，实验设置与完整结果参考此处。",
    ...
    },
    {
      ...
      "paras": [
        ...
        "<span style=\"display:block;background:#f5f8fb;border-left:3px solid #0F4C81;padding:10px 14px;border-radius:4px;\">当然，即便工程实现正确，改变 batch 形状也可能改变 kernel 选择或浮点归约顺序，使同一输入产生略有差异的数值结果，PyTorch 的数值精度说明也专门讨论了批量计算与逐项计算的差异。**Batch invariance 指同一输入的计算结果不随 batch 大小或组成变化；这里需要警惕的是缺乏这种不变性。** 这类差异不限于 neural encoder，也可能出现在 decoder、Flow 或 vocoder 中，但数值不同不必然意味着质量下降，更不能直接解释本次的 WER outlier，仍需对具体路径和样本进行验证。关于推理数值差异的相关讨论，也可以参考我们团队的文章RL 训推不一致的根源其实在 inference 系统中广泛存在。</span>",
    ...
    {
      "type": "h3",
      "title": "把可靠的测量方法沉淀进 CI",
      "paras": [
        "这些要求最终都应该进入我们的日常开发流程。PR 需要让 reviewer 知道改动了什么、与哪个基线比较、输入和环境是什么、预热与污染轮次怎样处理，并能沿着日志找到实际 batch、分组和返回行为。像此前的 TTS 重构把重复的生命周期管理沉淀进框架一样，我们也希望把可靠的测量方法沉淀进 benchmark 和 CI，让下一位贡献者能够复用，而不必重新猜一遍哪些条件会影响结果。",
        ...
        "如果你也对这样的工作感兴趣，非常欢迎参与 SGLang Omni 的开发。可以从复现一组跨硬件的对照开始，也可以补全一段 GPU 时间线，研究 reference 分布对 batch 的影响，或者用带时间戳与复现步骤的 issue，验证客户端补发节奏是否会让实际 batch size 周期性重复。这里既需要熟悉 kernel 的朋友，也需要愿意认真检查 benchmark、异常样本和调度细节的朋友。",
  ...
  "conclusion": [
    "**性能优化最大的敌人，是把没有控制变量的变化当成收益。** 把样本固定到 200 条、每次剔除 50 条预热、绑死 CPU 核区并做争用采样之后，FIFO batch size = 8 相对 batch size = 1 的提升才第一次具备解释力。",
    "两条更聪明的路径都没有赢：reference-aware 把外层调用从约 30 次推到约 137 次，early-emit 让提前返回的新请求回头抢占后续 group 的 GPU。GPU utilization 从约 74% 涨到约 89%，吞吐反而掉了约 17.5%。",
    "做 serving 的同学更该带走的是报告方式：「变快了多少」要和「原本会波动多少」一起给出；敢把参数设成默认值的底气来自可复现的证据，而不是一次漂亮的曲线。"
  ],
  "reference_url": "https://x.com/GenAI_is_real/status/2101785062157193316",
  "title": "从一次 Batch Size 争论，思考 SGLang Omni 的性能验证与调度取舍"
}
out_path = os.path.join(_article_dir, 'article_data.json')
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(DATA, f, ensure_ascii=False, indent=2)
print('OK sections=%d paras=%d' % (len(DATA['sections']), sum(len(s['paras']) for s in DATA['sections'])))
