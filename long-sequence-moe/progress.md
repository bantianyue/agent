# 进度追踪 - long-sequence-moe

| Step | 状态 |
|------|------|
| Step 0: 目录创建与TASKS.md | completed |
| Step 0a: 语言类型判断（英文→翻译） | completed |
| Step 0b: 来源类型确认（arXiv HTML） | completed |
| Step 1: 内容提取（全文） | completed |
| Step 2: 全部图片下载（x1-x10） | completed |
...
| Step 4d-i: 写传送门（published_articles.json选4-8篇） | completed |
| Step 4e: 写参考区 | completed |
| Step 4f: Humanizer 润色 | completed |
| Step 4g: 文本格式修复 | in_progress |
