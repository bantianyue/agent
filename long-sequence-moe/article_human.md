...
程序验证器在训练编码Agent中扮演着核心角色：无论是为SFT筛选高质量轨迹，还是为RL提供奖励信号，验证器决定了Agent的rollout是否成功解决了Issue。

目前，验证的黄金标准是执行测试：在每个仓库的Docker环境里运行单元测试。看着简单，实际工程成本高：为每个仓库定制Docker镜像、解析依赖、识别相关测试、编写执行脚本和结果解析器。自动化流水线只能覆盖有限比例的代码仓。根本问题是：大量真实世界的代码库，尤其是私有、企业或遗留仓库，根本没有可复现的环境或完整的测试套件。

通过共享基础镜像执行Agent rollout能降低部分成本，但验证器仍然是瓶颈。现有无环境验证器只靠表面信息（文本diff相似度、LLM一次性评分）来打分，而不检查代码库本身。对于复杂的软件工程任务，这种浅层方法远远不够：判断一个补丁是否真正修复了Issue，需要深入的代码仓上下文。
...
**第二阶段：判定。** 所有子Agent返回答案后，Dockerless聚合收集到的证据，输出一个二值判决token（0=不解决Issue，1=正确解决）。推理时将两个判决token的logits通过softmax转换为连续分数，作为评分。
...
训练使用拒绝采样。数据来源于SWE-Gym和Multi-SWE-RL的3,700个Issue，每个训练样本包含：Issue、参考补丁、候选补丁、真实标签。
...
**无环境RFT:** Rejection-sampling fine-tuning在最小镜像中收集大量rollout（16K），用Dockerless给每个rollout的最终补丁打分，保留top 4K用于SFT微调。这替代了传统的"通过仓库级测试才保留"模式。

**无环境RL:** 用Dockerless作为奖励模型跑GRPO。每次对同一个补丁执行M=2次独立的Dockerless评估并取平均，提升奖励稳定性。全程不涉及任何仓库级测试执行。
...
Dockerless在K=4时达到AUC峰值81.0。超过4个问题后性能不再提高（K=6时79.6，K=8时80.3），说明**额外问题会引入冗余或噪声证据。因此推理时生成2-4个问题，在准确率和每次调用的探索成本之间取得平衡。**
...
**验证这件事，可以不依赖执行环境。** 在编码Agent的训练流程中，搭建Docker环境一直是最麻烦的环节之一。Dockerless用"智能体主动探索代码库"替代了"跑测试"，在验证器自身性能上大幅领先现有方案，同时让整体post-training效果几乎无损。

把验证器本身也做成了一个Agent，让它像人一样去代码里找证据。这可能是一个比"提升验证器准确率"更根本的视角转换：不再问"这个补丁能不能通过测试"，而是问"我们能不能找到证据说明这个补丁是对的"。对于大量没有测试覆盖的真实代码仓来说，后一个问题显然更有实际意义。

<div style="background:#f5f0eb;padding:14px 16px 10px 16px;border-radius:6px;margin-bottom:16px;">
<div style="text-align:center;margin-bottom:8px;">
<strong style="font-size:15px;color:#8b6f4c;">真的是结语</strong>
</div>
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
Dockerless打开了"无环境post-training"这个方向：在那些没有测试套件、没有可复现环境的真实仓库长尾上，仍然有路可走。Agent化的验证器，可能是编码Agent从"比赛级"走向"工业级"的关键一块拼图。
</div>
</div>
