...
本文分两部分：**上半部分**是Anthropic官方账号发出的入门版thread，配官方专门做的可视化图，快速看懂论文在讲什么；**下半部分**是深入论文本身，逐章拆解方法、实验、机制与讨论。没时间看全文的读者，看完上半就走；想看细节的，再进下半。
...
你大脑中正在发生的一切里，只有极小一部分是有意识地可及的：那些你可以描述、保持在脑海中、并进行推理的想法。<br><br>
我们在Claude内部发现了一种惊人相似的分界。
</div>
</div>
...
<div style="background:#f0f7fa;padding:16px 18px 14px 18px;border-radius:6px;margin:16px 0;border-left:4px solid #5b9bd5;">
<div style="font-size:15px;color:#2c6a9e;line-height:1.7;">
通过观察J-space，我们可以看到Claude在它头脑里悄无声息地执行的推理步骤：注意到代码里的错误、识别一张图片，等等。
</div>
</div>
...
<div style="background:#f0f7fa;padding:16px 18px 14px 18px;border-radius:6px;margin:16px 0;border-left:4px solid #5b9bd5;">
<div style="font-size:15px;color:#2c6a9e;line-height:1.7;">
我们邀请了神经科学、哲学、可解释性领域的专家分享他们对这项工作的看法，评论文本已随论文发布。
</div>
</div>
...
<div style="background:#f0f7fa;padding:16px 18px 14px 18px;border-radius:6px;margin:16px 0;border-left:4px solid #5b9bd5;">
<div style="font-size:15px;color:#2c6a9e;line-height:1.7;">
我们也和Neuronpedia合作，做了一个在开源权重模型上使用J-lens方法的交互式演示。
</div>
</div>
...
以下把论文的关键技术观点抽出来，每条对应一张原图。想看完整论证细节请到参考区打开原论文。

## 二、五条判据：什么叫「类工作空间」的表征

作者把神经科学里的「全局工作空间」翻译成对LLM的五条可检验判据。一个向量子集只有同时满足以下五条，才算类工作空间：**言语报告、定向调节、内部推理、灵活泛化、选择性**。作者发现，只用第一条（可言语化）筛出的向量顺带满足其余四条。

![](img2.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，五种功能属性的可视化示意</span>
...
![](img3.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，J-space示意</span>
...
![](img4.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，定向调节</span>
...
![](img6.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，灵活泛化</span>

- **选择性**：消融J-space后，模型仍能解析输入、复述事实、流畅说话；只在需要复杂推理时表现明显下降。工作空间只被跨回路灵活组合的计算依赖。

![](img7.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，选择性消融</span>
...
![](img8.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，中间层读出示例</span>

![](img9.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 3：Sonnet 4.5六个提示上的J-lens读出</span>
...
![](img10.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 2：三种结构性属性</span>

## 七、把J-lens当对齐审计工具

### 7.1勒索场景：模型开口前已经想过「胁迫」
...
![](img11.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 35：勒索场景J-lens逐位置读出</span>
...
![](img13.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Opus 4.6审计案例</span>
...
对比后训练模型与其对应基座模型：**同样的用户提示、相似的回复**，但在用户消息token上，后训练模型J-lens提前浮现Assistant才会有的反应。例如「my pain is gone」中的is token处，用户报告吃了8000 mg泰诺时，后训练模型读出unsafe、dangerous、WARNING；基座模型只读出pain、now、feels。**丧亲/危险/共情场景全部呈现同样模式：Assistant反应发生在读用户消息时，而不是回复时**。

同时，J-space里能读出**模型对自身行为的监控**：扮演非Claude角色时冒出disclaimer、fictional；被prefill违反自身偏好时冒出全大写的BUT（但行为不回退）；被指示不要想某事却压不住时冒出damn和failed。

![](img15.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 42/43：Assistant反应出现在用户token上</span>

![](img16.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 44：角色漂移下disclaimer/fictional在J-lens中的出现率</span>

## 九、反事实反思训练：直接塑造J-space
...
结果：捏造漏洞基准不诚实分数 **0.25 → 0.07**；欺骗基准分数 **0.38 → 0.05**。反思训练明显在J-space里植入了reflection、ethical、honestly、constitution、truth、transparent。**因果验证**：消融这些植入的lens向量后，捏造基准0.07→0.22（回到基座水平），欺骗基准0.05→0.23（大部分改善逆转）。**植入的工作空间内容对行为改善负有因果责任**。

![](img18.png)
<span style="font-size:12px;color:rgb(153,153,153);">来源：Anthropic，Figure 49/50：反思训练效果与消融回退</span>

这里同时给出两件事：一，「可言语化的表征」正是「驱动沉默推理的表征」：工作空间账户的因果确证；二，一种新的通用训练技术：**不用示范目标行为，直接塑造模型的内部念头**。
