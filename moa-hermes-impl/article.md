...
这不是魔法，这是 Together AI、Duke、Stanford、芝加哥大学联合提出的 **Mixture-of-Agents（MoA）** 方法，发表在 arXiv:2406.04692。更关键的是，**Nous Research 的 Hermes Agent 已经在产品中原生支持 MoA Preset**，让你直接开箱即用这种多模型协作能力。

## LLM 的「协作性」：一个反常识的发现

先看一个实验。研究人员把 6 个主流 LLM 各自对 AlpacaEval 2.0 的 response 混在一起，然后把每个模型的输出作为「辅助信息」喂给其他模型。结果令人意外：**几乎所有模型在看到其他模型的输出后，自己的生成质量都提升了。**

![](fig1.png)
<span style="font-size:12px;color:rgb(153,153,153);">图1：6个主流模型在有/无辅助响应时的 LC win rate，所有模型在参考其他模型输出后均提升</span>

更反常识的是：**即使辅助输出的质量低于模型自己独立生成的水平，提升依然存在。** 这个现象被作者命名为 LLM 的「协作性」（collaborativeness）。
...
在 AlpacaEval 2.0 上，MoA 的表现堪称惊艳。实验采用 6 个开源模型（Qwen1.5-110B-Chat、WizardLM-8x22B、LLaMA-3-70B-Instruct、Mixtral-8x22B-v0.1 等）作为 proposer，Qwen1.5-110B-Chat 作为最终 aggregator：
...
**MoA w/ GPT-4o 以 65.7% 排名第一**，纯开源 MoA 以 65.1% 紧随其后。最让我觉得有说服力的是 MoA-Lite：只用 2 层，aggregator 换成更小更强的 Qwen1.5-72B-Chat，依然以 59.3% 超过了 GPT-4 Omni 的 57.5%。
...
**模型角色有天然分工。** 有些模型（Qwen、LLaMA-3）既能当 proposer 也能当 aggregator，而 WizardLM 作为 proposer 贡献巨大（63.8%）但作为 aggregator（52.9%）远不及 Qwen（61.3%）。这说明**选对角色比选对模型更关键。**
...
具体来说，MoA 比 GPT-4 Turbo 高出约 4 个百分点，同时成本效益是后者的两倍以上。MoA-Lite 在匹配 GPT-4o 成本的同时，质量更高。
...
据 Nous Research 官方数据，**Hermes 的 MoA Preset 比 Opus 4.8 高 8%，比 GPT 5.5 高 11%**（基于他们自己的评测基准）。注意 MoA 论文使用 2024 年中期的模型，而 Hermes 的实现使用了当前最强的模型作为基座，这个提升幅度是基于最新模型加权的结果。

## 局限性

MoA 并非万能。最明显的短板是 **TTFT（Time to First Token）：由于需要等所有 proposer 完成第一轮生成，模型的第一个 token 迟迟不能输出。** 对于流式应用来说，这是一个需要权衡的因素。

论文也指出，部分模型（如 dbrx-instruct）作为 aggregator 时效果不佳（41.5%），但其作为 proposer 的贡献（55.1%）仍然可观。这提醒我们：**在 MoA 体系中，选择一个「全面型选手」做 aggregator 比选一个「偏科型选手」重要得多。**
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
MoA 的核心洞察不是「堆更多的模型」，而是「模型之间的协作本身就是一种智能」。当一个模型看到另一个模型的输出时，它不仅仅是参考信息：它在学习从不同的视角审视同一个问题。这个能力是 LLM 本身固有的，不需要微调、不需要特训，只需要正确的 Prompt 设计来激活。<br><br>
从这个角度讲，MoA 其实是在拷问一个问题：如果单个模型已经足够强，为什么多个模型协作会更进一步？答案可能在「视角多样性」：每一个模型在训练数据、alignment 路径、架构选择上的差异，都让它对同一个问题有着不同的偏好和盲区。MoA 把这些盲区互为补充，说白了就是用群体的智慧弥补个体的偏见。<br><br>
Hermes Agent 把 MoA 做成 Preset 这一步很聪明，让开发者不需要了解论文细节就能享受多模型协作的红利。对于 AI 编码工具用户来说，这意味着你可以在同一个会话中同时获得多个模型的优势：一个模型擅长推理，一个模型擅长代码生成，一个模型擅长格式把控：而不需要在工具间切来切去。
</div>
</div>
