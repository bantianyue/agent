...
The fused path also drops the Hadamard transform. Applying the same orthonormal transform to Q and K preserves their inner products before quantization, so its main effect was on the quantized representation. The fused path instead quantizes the untransformed activations directly.
