DATA = {
    "summary": [
        ...
        {"key": "IndexShare", "body": "让 MTP 复用 DSA 索引器 top-k，长上下文下 draft 开销最多降 1.9 倍。"},
        {"key": "TopK-V2", "body": "把 TopK 当选择问题，80K ISL 内核延迟降 2.33 倍，1M ISL 达 10.17 倍。"},
        {"key": "内核融合", "body": "索引器前导融合把内核数从 12 降到 4；CuteDSL BF16 GEMM 在解码小批量下胜 cuBLAS。"},
    ],
    "lead": [
    ...
    "conclusion": [
        ...
        "值得注意的关键判断是：加速几乎全部来自容量与调度的改善，而不是单点暴力。Spec V2 吃掉气泡、IndexShare 避免重复计算、TopK-V2 把排序降格为选择、前导融合砍掉启动开销，而每一处省下的都是原本就不存在于计算里的浪费。",
