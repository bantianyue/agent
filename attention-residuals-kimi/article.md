...
- 十年假设被打破：标准残差连接把每一层输出等权相加（权重恒为1），Kimi AI 的 Attention Residuals 用可学习、随输入变化的加权求和取代它。- PreNorm 稀释：深层模型里累积态随层数线性膨胀，第 N 层自身贡献被压到 1/N，大量层被模型学会直接忽略。- 跨层注意力：把 Transformer 解决序列问题的思路（注意力）搬到深度维度：每层用查询向量对所有历史层输出做内容感知的检索。- Block AttnRes 落地：分块压缩层历史，显存与跨机通信从随层数增长降为随块数增长；训练开销- 实打实的收益：同等质量下只需标准基线约 80% 的训练算力；GPQA-Diamond 推理任务涨 7.5 分，多步推理提升最明显。
...
Full Attention Residuals：每层用可学习的查询向量对所有历史层输出做加权求和
...
Block AttnRes：把层分成若干 block，只在 block 级摘要之间做注意力，大幅降低显存与通信开销
两阶段计算策略
推理时，朴素实现会在每一层都重算一遍完整注意力，代价很高。Kimi 团队用两阶段规避：
...
两阶段计算策略：Phase 1 批量处理 block 摘要注意力，Phase 2 顺序处理块内注意力
结果：数字说明了什么
...
标准残差连接作为 Transformer 设计的固定假设已历时十年。Attention Residuals 不是把它扔掉，而是把它推广：用对所有历史层输出的、可学习且随输入变化的加权求和，取代固定的等权累加。这套机制新增参数极小（每层一个向量加一次归一化），兼容现有架构，在模型规模和任务上带来一致提升。Block AttnRes 通过把层历史压缩成 block 级摘要，把训练开销压在 4% 以内、推理开销压在 2% 以内，才让它真正可扩展。围绕增量跨机通信和两阶段计算策略的工程工作，是把一个理论上站得住的想法，变成能在分布式训练集群上高效运行的东西。论文已挂在 arXiv，实现在 GitHub。对做 LLM 训练管线的工程师来说，这是一个有具体证据支撑、值得现在就搞懂的架构改进。
