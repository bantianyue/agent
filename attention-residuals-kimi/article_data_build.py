DATA = {
    'title': 'Kimi AI 的 Attention Residuals：修掉 Transformer 沿用十年的残差假设',
    'summary': [
        {'key': '十年假设被打破', 'body': '标准残差连接把每一层输出等权相加（权重恒为1），Kimi AI 的 Attention Residuals 用可学习、随输入变化的加权求和取代它。'},
        {'key': 'PreNorm 稀释', 'body': '深层模型里累积态随层数线性膨胀，第 N 层自身贡献被压到 1/N，大量层被模型学会直接忽略。'},
        {'key': '跨层注意力', 'body': '把 Transformer 解决序列问题的思路（注意力）搬到深度维度：每层用查询向量对所有历史层输出做内容感知的检索。'},
        {'key': 'Block AttnRes 落地', 'body': '分块压缩层历史，显存与跨机通信从随层数增长降为随块数增长；训练开销<4%，推理开销<2%。'},
        {'key': '实打实的收益', 'body': '同等质量下只需标准基线约 80% 的训练算力；GPQA-Diamond 推理任务涨 7.5 分，多步推理提升最明显。'},
    ],
    'lead': [
    ...
    'sections': [
        {
            'type': 'h2',
            'title': '残差连接在 Transformer 里到底做什么',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': 'Full Attention Residuals 怎么运作',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': '工程难题：Full AttnRes 为何无法直接扩展',
            'paras': [
    ...
    'conclusion': [
        '标准残差连接作为 Transformer 设计的固定假设已历时十年。Attention Residuals 不是把它扔掉，而是把它推广：用对所有历史层输出的、可学习且随输入变化的加权求和，取代固定的等权累加。',
        '这套机制新增参数极小（每层一个向量加一次归一化），兼容现有架构，在模型规模和任务上带来一致提升。Block AttnRes 通过把层历史压缩成 block 级摘要，把训练开销压在 4% 以内、推理开销压在 2% 以内，才让它真正可扩展。围绕增量跨机通信和两阶段计算策略的工程工作，是把一个理论上站得住的想法，变成能在分布式训练集群上高效运行的东西。',
        '论文已挂在 arXiv，实现在 GitHub。对做 LLM 训练管线的工程师来说，这是一个有具体证据支撑、值得现在就搞懂的架构改进。',
    ],
    'reference_url': 'https://datasciencedojo.com/blog/attention-residuals-kimi-ai-explained/',
}
