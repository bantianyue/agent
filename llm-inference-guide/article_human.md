...
从你的prompt到第一个token流出，中间发生的一切，可以拆成几个基本环节：分词、嵌入、注意力、Prefill/Decode的切分、KV缓存，以及量化。

你打了一个prompt。几百毫秒后，文字开始一个接一个地往外蹦。看起来很简单，其实完全不是。

在你的按键和第一个token之间，是现代计算里工程得最精细的一条流水线。诡异的是：模型为了回答你，在同一块GPU、同一次请求里，干了两件完全不同的活，带着两个不同的瓶颈。
...
这才是完整的循环：预测一个token，拼到末尾，再预测下一个。
...
生成一个200 token的回复，不是一项任务，而是两件看起来天差地别的任务。想清楚这一点，你才看得懂后面那些延迟数字到底在说什么。
...
Transformer架构吸引了所有的目光，但推理性能的生死却系在那些无聊的东西上：内存布局、cache管理、比特宽度。能不能把硬件榨干，决定了同样一个模型跑起来是快还是慢。下次调推理参数时，先想清楚瓶颈到底卡在算力还是带宽，往往比换更大的模型管用。
