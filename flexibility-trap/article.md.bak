...
那么，被绕过最多的token是哪一类？答案是逻辑连接词："Therefore"、"Thus"、"Since"、"However"等。先前研究已经指出，这类token具有高熵，充当推理路径的"分叉点"：不同的选择将牵引整个推理轨迹走向完全不同的方向。在传统语言模型中，保持这些分叉点的高熵状态对探索推理空间至关重要。
