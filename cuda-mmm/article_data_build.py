DATA = {
    'summary': [
        ...
        {'key': '算术强度', 'body': '核心思路是让每个线程算更多结果，提高算术强度（FLOPs/Byte），把瓶颈从访存推向计算。'},
        {'key': '可复现', 'body': '全部 kernel 代码开源，配合 Godbolt 看 PTX/SASS、Excalidraw 画图，是理解 GPU 微架构的最佳练手。'},
    ],
    'lead': [
    ...
    'sections': [
        ...
        {
            'type': 'h2',
            'title': 'Kernel 2：全局内存合并（Coalescing）',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': 'Kernel 3：共享内存缓存块（Cache-Blocking）',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': 'Kernel 6：向量化 SMEM 与 GMEM 访问',
            'paras': [
                ...
                '这让 32 位的 GMEM 加载指令（LDG.E、STG.E）被替换成 128 位的版本（LDG.E.128、STG.E.128）。作者起初困惑：为什么用 reinterpret_cast<float4*> 强制向量化，会比手动展开访问（或 pragma unroll）更快？原因大概是编译器无法验证传入 kernel 的 float* B 指针是否 128 位对齐——而这是使用 LDG.E.128 的前提，所以 reinterpret_cast 替它做了这个保证。',
        ...
        {
            'type': 'h2',
            'title': 'Kernel 9：自动调优（Autotuning）',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': 'Kernel 10：Warptiling（Warp 级分块）',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': 'Work in Progress：Kernel 11 与结语',
            'paras': [
