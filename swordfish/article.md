...
Swordfish已集成进Sonar（前Aphrodite Engine），添加于 `dphnAI/sonar#1707`，在支持的硬件和检查点上自动选中，也可强制 `--linear-backend swordfish` 和 `--moe-backend swordfish`。它构建于Elias Frantar等人的Marlin、Neural Magic团队的Machete、NVIDIA的CUTLASS以及qutlass项目之上。所有测量均使用锁定时钟、冷权重轮转、在B200和Jetson Thor开发套件上重复运行的中位数计时。
