...
- 追上显式CoT：在Llama-3.2-3B-Instruct上，LOTUS把GSM8K域内差距缩到1.5分以内，域外平均反超显式CoT，是首个在3B规模弥合隐式-显式差距的方法。- 思考快2.5–6.9倍：把N步顺序生成的CoT压缩为R≪N次并行循环迭代：数学设定思维快2.5倍，自然语言CoT压力测试快6.9倍。- 循环填充式骨干：在问题-答案间插入K个可学习隐式块，复用同一套权重循环R次，不增参数即获得计算深度。- 并行CoT监督：直接经基础LM头用交叉熵把每个隐式位置对齐到金标准CoT token，简单且跨规模稳健；辅助解码器路由在3B上与之持平。- 隐空间可读：循环后隐状态可直接读出金标准推理步骤，还能浮现未见但有效的备选中间路径，证明隐状态真正与CoT对齐。

扩展推理计算量，也就是让模型在回答之前先「思考」，已经成为提升语言模型能力的主要手段。如今更强的性能来自更长的推理链，而非单纯依靠模型规模（DeepSeek-AI, 2025；OpenAI, 2026）。思维链（Chain-of-thought, CoT）推理（Wei et al., 2022）让模型生成中间推理步骤，推动了这一趋势。然而，由于每个token都必须顺序解码，生成长度为N的CoT需要N次顺序的模型前向计算，使得推理成本高昂。
隐式推理（latent reasoning）旨在以极低的成本实现同样的效果：它在连续的隐藏状态而非解码出的token中完成中间计算，把多步运算压缩进更少的前向计算里。在GPT-2这类小骨干网络（Radford et al.）上，隐式方法（Hao et al., 2025；Shen et al., 2025；Wei et al., 2025）已经能和CoT的准确率持平。然而在CoT最关键的规模上，这一前景却失效了：超过11B参数后，没有任何现存的隐式方法能在数学推理上匹敌显式CoT，而且差距随模型规模增大而拉大（Wei et al., 2025）。

1引言
我们归纳出隐式方法中两个普遍存在的问题。（P1）顺序生成：Coconut（Hao et al., 2025）、CODI（Shen et al., 2025）和SIM-CoT（Wei et al., 2025）这类方法以纯自回归的方式产生隐式思维，因此顺序前向计算的次数仍然随隐式token数量线性增长，在保留CoT顺序生成瓶颈的同时，又放弃了显式CoT所提供的可读中间步骤。（P2）缺乏CoT锚定：显式CoT给每个推理步骤一个直接的、位置对齐的token目标。如果没有类似的锚定监督，隐式轨迹就可能偏离有意义的运算，并在大规模训练时变得不稳定（Li et al., 2026b；Zou et al., 2026）。PCCoT（Wu et al., 2025）和KaVa（Kuzina et al., 2026）用Jacobi式的迭代缓解了顺序瓶颈，但仍通过间接信号来锚定隐状态，达不到CoT对齐的目标，比如来自教师CoT模型的隐藏状态蒸馏，或压缩过的教师键-值缓存。
一个能同时解决这两个问题的架构，应当用少数几次并行前向来精炼隐状态，而不是每个token走一次自回归步骤，并且应当用和显式CoT一样直接的监督目标来锚定它。循环（looped），或称循环深度（recurrent-depth）Transformer天然契合第一个要求：它们跨迭代复用同一套权重来增深计算深度，而无需额外参数；近期工作已把这种设计扩展到十亿参数的预训练（Geiping et al., 2025a；Zhu et al., 2025c；Zeng et al., 2026b）。在可并行的问题上，它们能用比CoT更少的模型迭代次数可证明地达到解（Saunshi et al., 2025）；例如，LPT能在对数级的迭代次数内解决图可达性问题（Merrill and Sabharwal, 2025a），相比顺序隐式方法是指数级的提升。这引出了我们研究的问题：循环Transformer能否在更少的顺序步骤内推理的同时，弥合与显式CoT的差距？
我们发现，把一个循环填充式骨干网络与对金标准CoT token的简单监督配对，效果出奇地好。我们提出LOTUS（Looped Transformers with parallel supervision on latents，带并行监督的循环Transformer）。LOTUS在问题和答案之间放置K个可学习的填充式隐式块，每块含c个token，并用基础模型前向R次处理该序列。因此长度为N的显式CoT是在R ≪ N次密集并行计算迭代中被处理的，而不是N次顺序的逐token生成，这就解决了P1。为应对P2，LOTUS通过基础模型自身的LM头直接监督隐状态：一个交叉熵损失把每个隐式位置与其对应的金标准CoT步骤token对齐，且全部并行完成。该监督目标也可以经由一个辅助解码器来路由，这个解码器以所有隐状态为条件，在教师强制下为金标准CoT token打分：我们把这种变体实例化在和LOTUS相同的循环骨干上，称为LOTUS-aux。两种路由在3B规模上都达到了接近CoT的准确率。据我们所知，LOTUS是首个在Llama-3.2-3B-Instruct规模上弥合数学推理域内差距、追上显式CoT的隐式推理方法（图1），同时在域外平均上超越CoT，并把思考时间缩短2.5倍。在一个更冗长的自然语言CoT压力测试中，LOTUS的准确率与显式CoT持平，而把思维阶段延迟降低了6.9倍。LOTUS的隐空间也是可解释的：通过基础LM头读取循环后的隐状态，能还原出金标准推理步骤，甚至能浮现出模型从未训练过的、有效但未见过的备选中间步骤，这证明隐状态是真正与CoT对齐的，而非不可穿透的黑箱。我们的贡献总结如下：
我们提出了「循环填充式Transformer + 对金标准CoT token的并行交叉熵监督」这一范式。该方法使用一个填充式隐式前缀（第3.1节），用循环骨干精炼它（第3.2节），并通过基础LM头监督循环后的隐状态（第3.3节）。我们还实例化了LOTUS-aux，它把同样的隐式监督经由一个仅在训练时使用的辅助解码器来路由（第3.4节）。
我们证明，据我们所知LOTUS是首个在3B规模上弥合数学推理隐式-显式CoT差距的方法：在Llama-3.2-3B-Instruct上，它把GSM8K测试准确率拉到距显式CoT仅1分以内，并在域外数学平均上超越显式CoT。它还在数学表达式设定下把思维阶段延迟降低2.5倍，在自然语言CoT压力测试中降低6.9倍（第4.2节）。
消融实验表明，并行监督设计（第4.3.1节）和循环架构设计（足够的块宽度和循环深度，第4.3.2节）都很重要。直接的LM头路由在跨规模上稳健，而辅助解码器路由在3B上与之持平，但在更小骨干上会退化（第4.2节）。
LOTUS产生了一个透明、与CoT对齐的隐空间。读取循环后的隐状态能还原大部分金标准中间token（第5.1节），同时把不可忽略的概率质量分配给未见但有效的备选（第5.2节）。一次损失消融显示，CoT监督把读出锚定到金标准链，而答案监督帮助选出连贯的联合解（第5.3节）。
2预备知识
全部符号的术语表见附录B的表12。
一个语言模型（LM）f_θ 可以通过多步推理把问题Q映射到答案A：在答案之前生成S个中间步骤T_1, …, T_S，其中T_i（i ∈ {1, …, S}）可能跨多个token。我们用N表示全部S个步骤中CoT token的总数。标准CoT监督通过教师强制并行训练所有CoT token，但在推理时这些token必须顺序生成，增加了延迟。我们把这种设定称为显式CoT。
隐式推理用模型隐藏状态中的多步推理替代解码出的CoT token，这些隐藏状态通常被称为隐式思维（latent thoughts）。Zhu et al. (2025a) 证明隐式思维携带的信息严格多于离散token：Transformer中线性数量的隐式步骤就能解决图可达性问题，而显式CoT需要二次数量的步骤。
循环Transformer（Dehghani et al., 2019）多次应用f_θ 的骨干网络，在不增加参数的前提下获得深度。用额外的可学习token填充输入（Goyal et al., 2024；Pfau et al., 2024）给每次迭代增加了并行计算。两者的结合构成了循环填充式Transformer（LPT），其能力优于任一单独组件。
我们把LOTUS与最相近的隐式推理方法对比，并在这里总结（细节见附录C）。Coconut（Hao et al., 2025）在课程学习下用c个隐式token自回归地替换每个CoT步骤，并只监督答案。CODI（Shen et al., 2025）保留这种自回归隐式解码，但增加了一个蒸馏目标，把一个指定隐状态与教师CoT模型对应的隐藏状态对齐。SIM-CoT（Wei et al., 2025）则训练一个辅助自回归解码器，把每个隐状态与其CoT token对齐。PCCoT（Wu et al., 2025）和KaVa（Kuzina et al., 2026）去掉了自回归性质，在几次Jacobi迭代中并行精炼固定预算的隐式token；它们分别通过CODI式蒸馏和压缩的教师键-值缓存间接锚定隐状态。关于隐式推理和循环Transformer的相关工作的延伸讨论见附录A，与各隐式推理方法的详细对比见附录C。
3方法
尽管LPT的表达能力足以进行高效的并行推理，但如果没有结构化监督，它们往往无法泛化到分布外（Altabaa et al., 2025）。这促使我们做出核心设计选择：把隐式计算直接锚定到金标准CoT token。我们用两个要素把一个标准LM变成隐式推理器：（i）一个由LPT处理的填充式隐式前缀；（ii）对精确金标准CoT token的并行交叉熵监督。我们把这个范式实例化为LOTUS（Looped Transformers with parallel supervision on latents，图2），它直接通过基础模型的LM头来读取监督。我们还研究了LOTUS-aux（图3），它改为通过辅助解码器来路由监督。我们在第3.1节详述填充式隐式前缀，第3.2节详述循环计算，第3.3节详述LOTUS的目标（含为何有效的分析，见第3.3.2节），然后在第3.4节引入LOTUS-aux变体。

Figure 2: LOTUS architecture. (a) Looped forward: the looped LM f_theta is iterated R times over [,  ⋯ , ], producing post-loop hidden states h^(R) at the latent positions, and L_step supervises these states through the LM head, in parallel, against the corresponding CoT tokens T_i,j. (b) Final forward: the post-loop latents h^(R) are inserted at the latent positions and the answer suffix A is supervised by next-token prediction (L_ans) through the LM head.
3.1填充式隐式前缀
对于问题Q、S个思维链（CoT）步骤T_1, …, T_S和答案A = (A_1, …, A_|A|)（其中A_|A| = ⟨EoS⟩），我们构造输入
其中 ⟨lat⟩ 是一个在位置间共享的可学习隐式token，⟨BoT⟩、⟨EoT⟩ 是标记隐式区域边界的可学习特殊token。块预算K和每块宽度c是固定超参，因此隐式区域包含K·c个隐式token加上两个边界符。该隐式前缀沿用先前关于填充或暂停token的工作（Goyal et al., 2024；Pfau et al., 2024），每个块i意图编码CoT步骤T_i的隐式表示，i ∈ {1, …, K}。与第2节中依赖于样本的步骤数S不同，块预算K在样本间固定；我们把它选得能覆盖数据中的步骤数，使得在几乎全部样本上K ≥ S，从而每个块可以对齐到一个CoT步骤（附录D）。
3.2循环隐式计算
设f_θ 为基础因果LM，E ∈ ℝ^{Kc×d} 为可学习的隐式嵌入，按块i ∈ {1, …, K} 和块内位置j ∈ {1, …, c} 索引为E_{i,j}。LOTUS先对前缀 [Q, ⟨BoT⟩] 做一次前向，填充KV缓存C_pre，它在训练和推理中全程复用而不重算。然后它迭代f_θ 共R次，每次精炼隐式嵌入，同时注意力到C_pre（尾部的 ⟨EoT⟩ 和答案后缀A不进入循环）。记h^(t) ∈ ℝ^{Kc×d} 为第t次迭代后隐式位置上的隐藏状态，其元素为h^(t)_{i,j}：
其中方括号表示子序列拼接。这是循环Transformer上的有限展开、输入注入式递推（Dehghani et al., 2019；Fan et al., 2025），具有固定的展开深度R，梯度穿过全部R次迭代反向传播。
3.3直接的步骤对齐监督
经过R次循环（第3.2节）后，我们用对应的CoT步骤token监督网格中每个隐式位置 (i, j)。令T_i = (T_{i,1}, …, T_{i,c}) 为token化后补零或截断到c个token的CoT步骤i。我们应用一个批量的交叉熵，直接把每个块与其目标步骤对齐：
其中f_head是LM头，N_step = Σ_i |T_i| 是被监督（非填充）CoT token的总数。用于把CoT步骤补零到长度c的填充token被交叉熵忽略（也不计入N_step）。
L_ans在f_θ 的一个独立最终前向中计算，它复用前缀缓存C_pre，并在隐式行上插入循环后的隐式隐藏状态h^(R)：
其中z_m ∈ ℝ^d收集每个答案后缀位置m ∈ {0, 1, …, |A|−1} 处得到的隐藏状态（m = 0为尾部 ⟨EoT⟩ 位置，它预测A_1）。这里我们用z表示隐藏状态，以区别于循环前向的隐式状态h^(t)，标明它们来自不同来源。应用LM头f_head，对答案后缀的标准下一token交叉熵给出：
给定步骤监督权重 λ_step，完整目标为
式6中的监督有三个性质：（i）直接：每个块的目标T_i通过用于生成答案的同一LM头打分；（ii）并行：全部K个块同时被监督；（iii）循环后：读出在最后一轮迭代后读取h^(R)，而非在每次迭代上。我们在第4.3节消融这一设计选择，并在附录C中与先前的隐式推理方法对比。
推理时，LOTUS运行循环前向，然后解码答案。我们一次性计算问题Q的KV缓存，迭代循环R次以获得循环后的隐状态h^(R)，然后通过基础LM头自回归地解码答案A，同时以这些隐状态为条件（与图2相同，只是不计算损失）。推理完全由并行隐式块承担，因此唯一的顺序解码发生在短小的答案后缀上，这正是第4.2节所报告延迟收益的来源。
注意，隐式推理步骤在答案生成过程中从不被读出。它们通过LM头投影到token空间，只是为了在训练时通过L_step与CoT推理对齐。答案始终由隐式隐藏状态生成，无论是在最终的L_ans前向（式4）还是推理时都是如此。
对LOTUS目标（式6）的一个自然担忧是，它独立地监督每个隐式位置：式3并行地为每个金标准CoT token打分，没有对链做自回归分解。为什么独立的逐位置目标能训练出一个产生全局连贯答案的模型？我们给出一个视角来化解这一担忧，区分两个损失的角色，并解释为何二者都必要。
步骤损失L_step在读出位置上监督一个并行链似然（PCL）。最大化每个金标准token在其隐式位置上的逐位置概率p_θ(T_{i,j} | Q)，诱导出链似然
而非自回归条件概率p_θ(T_{i,j} | Q, T_{
...
Figure 3: LOTUS-aux supervision at loop iteration t for block t. The auxiliary decoder g_phi only replaces the base LM head supervision in Figure 2(a).
...
令人惊讶的是，即便在LOTUS-aux中h^(R) 从未被直接监督通过g_head预测金标准CoT，我们仍能以25.8% 的top-5概率通过g_head读取h^(R)。贪心解码也浮现出一些有意义的金标准token，表明CoT内容由隐状态本身承载。尽管经过不同的头监督，LOTUS和LOTUS-aux有相似的效果，使循环后隐状态h^(R) 把质量放在CoT步骤上，但LOTUS-aux读出格式较乱（这符合预期，因为它未被直接监督）。我们在附录F中给出两个模型并排的h^(R) 读出示例。
...
Figure 4: Multi-path readout probability for the example in Section F.3. The only-U intermediates do not appear in the trained path, question, or answer, but they surface in the post-loop latent readout.
...
我们提出LOTUS，表明隐式推理可以通过用简单的交叉熵目标、并行地以金标准CoT token监督一个循环填充式Transformer，来逼近显式CoT的性能。在Llama-3.2-3B-Instruct上，LOTUS弥合了GSM8K上域内与显式CoT的差距，在域外平均上超越CoT，并把思维阶段延迟降低2.5倍。消融表明循环骨干、并行金标准CoT监督和足够的块宽度与循环深度各自都是必要的。隐式表示分析进一步表明隐状态是透明的：金标准CoT可通过直接读出从中还原，它们对未见但有效的推理链分配分级概率而非单一记忆轨迹，且步骤损失与答案损失贡献了互补的结构。我们沿用先前的隐式推理工作（Hao et al., 2025；Shen et al., 2025；Wei et al., 2025）在数学基准上评估。该范式能否迁移到其他领域仍是未来的开放方向。块预算K、每块宽度c和循环深度R是固定超参，必须设置得能覆盖预期步骤数，因此长于K步的链会回退到尾部的自回归补全。让K、c和R自适应是自然且有前景的未来方向。**独立观点**：LOTUS的意义不只在「快」，而在于它把隐式推理从黑箱变成了可读的对象。它用最朴素的交叉熵、最朴素的循环复用，就追平了显式CoT，而且隐状态里能读出模型从未被训练过的有效推理路径。这暗示一个大方向：很多「必须显式生成」的中间过程，其实可以在隐藏状态里并行完成，只要监督信号足够直接。
