...
DATA = {
    'summary': [
        {'key': '核心问题', 'body': '蒸馏后的少步扩散模型（Z-Image-Turbo、FLUX.2-klein）用常规 SFT 继续调，几乎必掉画质、丢失少步生成能力。'},
        {'key': 'D-OPSD 方案', 'body': '同一个模型分裂两个角色：学生只看文本，老师看文本+目标图，在学生自己走出的少步轨迹上把老师的速度场蒸馏给学生。'},
        {'key': '关键前提', 'body': '现代扩散模型的 LLM/VLM 编码器天然继承了上下文能力，把目标图当图文并列输入即可产生指向该概念/风格的变体，无需额外训练。'},
        {'key': '实验结论', 'body': 'LoRA 小样本定制、25K anime 全量微调两个场景下，D-OPSD 在 DINO-D、LPIPS-D、VLM-J、GenEval、DPG 上全面压过 Vanilla SFT / DreamBooth / PSO，用户偏好也显著更高。'},
        {'key': '额外成本', 'body': '每步训练大约 4× FLOPs、2× 墙钟时间，但相较于「SFT 再重蒸馏」的两阶段管线，总开销反而更省。'},
    ],
    ...
    'sections': [
        ...
        {
            'type': 'h2',
            'title': '关键观察：LLM/VLM 编码器带来的图文上下文能力',
            'paras': [
        ...
        {
            'type': 'h2',
            'title': '实验二：25K anime 全量微调 + 用户偏好评估',
            'paras': [
    ...
    'conclusion': [
        'D-OPSD 的实际价值在于把「少步蒸馏模型的持续微调」从一个悬而未决的工程问题，变成了一条**只需图文对、不需奖励模型、不需重蒸馏**的可复用管线。这一点对社区二次开发者尤其重要——过去大家在 Z-Image-Turbo 这类模型上做 LoRA 定制，几乎都要在「学到新概念」和「保住少步画质」里二选一。',
        '把「同一模型两个角色 + 学生自己 rollout + 目标图作强上下文」抽象出来看，这套路其实是 LLM 领域自蒸馏思路在扩散模型上的一次对应实现。作者也指出了几个自然延伸：把老师侧上下文换成图像编辑模型或视频生成模型的更强条件、结合 SRA/SoAR 之类的额外约束、以及在 D-OPSD 损失上叠多专家 OPD 做后训练。',
        '现阶段最直接的启示是：**当 LLM/VLM 成为扩散模型编码器之后，「一个模型同时充当学生和老师」不再需要额外结构改造**——上下文能力已经藏在编码器里，D-OPSD 只是把这条隐性能力显式利用了起来。少步扩散模型的持续学习，正在从「几乎不可能」变成「跟传统 SFT 一样标准化」。',
    ],

    'reference_url': 'https://arxiv.org/html/2605.05204v3',

    'caption_translations': {
        'fig00': '图 2：方法总览。训练每一对图文时，先把 prompt 单独和 prompt+目标图分别过编码器得到 c_s 与 c_t；学生分支用 c_s 采一条少步轨迹；老师和学生在同一条轨迹的每个状态上预测速度场，用 D-OPSD 损失更新学生。训练结束后丢掉老师分支，推理与原始少步文生图管线完全一致。',
        'fig01': '图 3：Z-Image-Turbo 上的 LoRA 定制视觉对比。Vanilla SFT 牺牲少步画质、PSO 过拟合训练集，D-OPSD 学到新概念的同时保住少步生成质量。',
        'fig02': '图 4：Z-Image-Turbo 上的全量微调视觉对比。SFT 与 PSO 同样丢失少步生成能力，D-OPSD 迁到目标域时保留了少步能力与原始知识。',
        'fig03': '图 5：与基线的人评对比。所有图像使用相同噪声。D-OPSD 在画质、美学、prompt 一致性三项上均优。',
        'fig04': '图 7：当老师模型在图文条件下无法生成概念一致的图，就没有有效监督信号，训练会失败。',
    },
}
