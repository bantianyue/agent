...
async function main() {
  const draftMediaId = fs.readFileSync(path.join(__dirname, "draft.id"), "utf-8").trim();
  ...
  if (!article) throw new Error("No news_item");

  fs.writeFileSync(path.join(__dirname, "server_content.html"), article.content || "", "utf-8");
  console.log(JSON.stringify({ title: article.title, content_length: (article.content || "").length }));
}
