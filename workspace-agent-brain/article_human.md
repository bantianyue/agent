...
反事实反思训练可能更值得盯——**这项内部技术如今已公开发表，意味着Anthropic在自己最前沿的模型上早已掌握并初步验证了其效果**。不训练目标行为本身，只训练模型在被追问时说什么，就能让原始语境下不诚实分数从0.25降到0.07。如果这种技术能被泛化到更抽象或更具体的倾向上，它就是一种直接在概念级别植入原则的路径。<br><br>
