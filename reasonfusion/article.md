...
- 叠加态机制：每个连续思维向量同时编码多个搜索前沿，等价于在图上做隐式并行广度优先搜索（BFS）。- D步 vs O(n²)步：两层层Transformer用D步连续思维（D为图直径）即可解有向图可达性；离散CoT最优已知结果是O(n²)步解码。- 2层胜12层：从零训练的2层连续CoT模型在图可达性上逼近满分，胜过12层离散CoT（83%准确率）。- 自动涌现：叠加态表示仅用最优路径监督即可从训练动力学中自发涌现，无需显式引导多路径探索。
...
图2：嵌入空间与第一层注意力机制的示意。每个注意力头是一个注意力选择器，把边token的源/目标节点信息复制到对应位置。
第一层注意力包含5个注意力头，每个头都是一个注意力选择器。它们把每条边token的源节点和目标节点信息拷贝到对应位置，为后续扩展做好准备。

图3：第二层注意力机制的示意。当前连续思维（叠加态）对源节点已在可达集合内的出边给予高注意力，并将目标节点加回思维，实现一步BFS扩展。
...
图7：第二层注意力机制的示意（预测阶段）。答案token「测量」叠加态，比较候选终点信号强弱以输出可达性判断。
实验验证：理论构造与训练结果高度吻合
...
图4：Coconut、CoT、CoT*（12层，12个注意力头）在有向图可达性上的总体准确率对比。Coconut接近满分，离散CoT即便加深也明显落后（随机猜=50%）。
...
图5：第一层注意力图的一个代表样例。边token  强烈关注其源节点 s 和目标节点 t，验证了拷贝机制的实例化。
...
图6：第i步连续思维与各节点嵌入内积的分布直方图。i跳内节点相似度明显更高，且前沿节点、最优节点依次更靠近当前思维。
...
这篇论文的价值在于把「连续思维比离散思维强」从一个经验现象变成了一个可证明的理论事实。叠加态视角解释了为什么连续潜空间能并行探索多条推理路径，而离散token被迫逐条串行。对工程实践的启发是：在需要多路径搜索、规划、组合推理的任务上，连续潜思维可能用远少的计算（层数、步数）达到更好的效果。而「叠加态自发涌现」这一发现尤其关键——它说明不需要精心设计的多路径监督，标准训练就能让模型学会并行搜索，这为更高效的推理模型设计打开了空间。
