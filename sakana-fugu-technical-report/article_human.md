...
不同LLM各有专长：GPT 在数学上强，Opus 在工程和安全上有专长，Gemini 在事实回忆上出色。一个直接的问题是：能不能让它们组合编排？

Sakana AI 今天发布了Fugu技术报告的完整版，这是他们学习型编排器模型的正式学术论文。Fugu 本身是一个经过训练的 LLM，能理解用户问题并动态设计 Agent 编排方案。
...
Fugu 的参数化设计很有意思：在编排器骨干的最终隐藏层后加一个轻量级选择头。给定隐藏状态 h，头输出 L 个 logits 为每个模型打分。Fugu 用 logits 而非生成文本做决策：系统只需要在早期 token 位置算个隐状态，应用选择头就完成分派，不需要跑完整自回归解码。这意味着 Fugu 的推理开销极低。
...
只使用单模型选择变体的 Fugu 在多个基准上也匹配或超越了单个前沿模型：Terminal Bench 上 80.2%。分析轨迹发现，Fugu 在一个问题的解决过程中在 GPT-5.5 和 Claude-Opus-4.8 之间交替，在关键的调试点换 Opus 上场。

科学推理方面，两者在 GPQA-Diamond 上达到 SOTA，甚至超越了不公开可用的 Mythos Preview 和 Fable 5 模型类别。这支持了 Fugu 的核心主张：智能编排可以作为独立于训练算力的性能轴，无需训练更大模型即可获得超越当前代际的能力。
...
**编排策略的发现：GPT 做建筑工、Opus 做质检员最有效**

Fugu-Ultra 在 Terminal Bench 中构建 PyPI 服务器时展示了教科书式的编排策略。它先部署 GPT-5.5 做建筑工：完成了服务器构建并确认可访问。然后部署 Opus-4.8 做质检员：Opus 发现了 GPT 实现中的三个问题：用了静态 http.server 而非 pypiserver、手建的 wheel 脆弱、Debian-slim 环境管理不当。这些问题回流给 GPT 后，GPT 成功完成了构建。
...
当任务需要密码分析领域的数学推导时，Fugu-Ultra 用 Opus 做第一版攻击方案构建，然后让 GPT 以数学专家身份从基本原理重新推导整个攻击，逐 bit 跟踪密码推导出差分常数。

这种「构建→质检→重构」的交替编排，单一的 Claude Code 或 Codex 无法复现。
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
Sakana Fugu 的技术报告没有隐瞒编排引入的延迟成本，也没有回避多 Agent 系统中的函数调用和记忆隔离难题。它主张编排应该是模型层面的能力，而不只是产品层面的功能。<br><br>
Fugu 与现有的"模型路由"类产品（如 OpenRouter）的区别在于：它不是用外部规则或轻量分类器做路由，而是用一个经过训练的 LLM 去理解问题内容，并基于隐藏状态做决策。编排策略本身也在持续适应：随着新模型加入池子、新基准出现、新用户需求产生，Fugu 可以不断调整。<br><br>
如果这个方向成立，AI 行业的竞技规则会发生变化：训练最大模型不再是通往前沿的唯一路径，学会如何组合现有模型将成为新的竞争壁垒。
</div>
</div>
