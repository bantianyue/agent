...
-核心机制：草稿模型一次猜出一小段token，目标模型一次并行校验，把逐token生成变成块生成-四代演进：EAGLE-3拉长接受长度，DFlash用并行草拟压缩草稿时间，DSpark削校验时间，DFlash 2再把接受长度拉回来-无损边界：拒绝采样只在算法层保证分布不变，阈值低于1.0、调度偷看、量化与内核差异都会让无损失效
...
题图：自回归画家一个像素一个像素地画；扩散画家一次把全部像素去噪，因此快得多。能不能用扩散的方式草拟token，一次给出一整块，而不是一个一个来？
...
图1：原始解码每经过一次目标模型的前向传播只输出一个token；投机解码让草稿模型先提出一小块，再交给目标模型校验。
拒绝采样：无损从哪来
...
图2：一次校验中的拒绝采样。每个草稿token以min(1, p/q) 的概率被接受；被拒绝的token从norm(max(0, p − q)) 重采样，草稿里剩下的部分全部丢弃。
同一条规则用伪代码写出来是：
#&nbsp;p(x):&nbsp;target&nbsp;model&#39;s&nbsp;probability&nbsp;for&nbsp;token&nbsp;x#&nbsp;q(x):&nbsp;draft&nbsp;model&#39;s&nbsp;probability&nbsp;for&nbsp;token&nbsp;xfor&nbsp;each&nbsp;draft&nbsp;token&nbsp;x:&nbsp;&nbsp;&nbsp;&nbsp;accept&nbsp;x&nbsp;with&nbsp;probability&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;min(1,&nbsp;p(x)&nbsp;/&nbsp;q(x))&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;q(x)&nbsp;&lt;=&nbsp;p(x):&nbsp;always&nbsp;accepted&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;q(x)&nbsp;&gt;&nbsp;&nbsp;p(x):&nbsp;accepted&nbsp;with&nbsp;probability&nbsp;p(x)/q(x)on&nbsp;the&nbsp;first&nbsp;rejection:&nbsp;&nbsp;&nbsp;&nbsp;discard&nbsp;the&nbsp;remaining&nbsp;draft&nbsp;tokens&nbsp;&nbsp;&nbsp;&nbsp;resample&nbsp;one&nbsp;token&nbsp;from&nbsp;the&nbsp;residual&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;p′(x)&nbsp;=&nbsp;norm(&nbsp;max(0,&nbsp;p(x)&nbsp;-&nbsp;q(x))&nbsp;)
从统计上看，一个token有两条被输出的路径：直接从草稿里被接受，这部分是被接受的质量；或者在拒绝之后由目标模型重采样，这部分是残差质量。两者相加正好等于目标模型自己的概率：
P(x&nbsp;is&nbsp;emitted)&nbsp;=&nbsp;q(x)&nbsp;*&nbsp;min(1,&nbsp;p(x)/q(x))&nbsp;&nbsp;&nbsp;#&nbsp;accepted&nbsp;mass&nbsp;=&nbsp;min(p(x),&nbsp;q(x))&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+&nbsp;P(reject)&nbsp;*&nbsp;p′(x)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;residual&nbsp;mass&nbsp;=&nbsp;max(0,&nbsp;p(x)&nbsp;-&nbsp;q(x))&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=&nbsp;min(p(x),&nbsp;q(x))&nbsp;+&nbsp;max(0,&nbsp;p(x)&nbsp;-&nbsp;q(x))&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=&nbsp;p(x)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;same&nbsp;as&nbsp;the&nbsp;target&#39;s&nbsp;probability
衡量加速的六个指标
到底快了多少，又该怎么衡量？表1定义了六个指标。其中三个是决定因素：草拟时间、校验时间、接受长度。另外三个，解码加速比、单token延迟和每秒token数，都由这三个因素算出来。

指标定义由什么决定

解码加速比 ηη = L(target) / L，相对自回归基线的提速倍数单token延迟L与基线延迟L(target)
单token延迟LL = (T(draft) + T(verify)) / τ，生成每个token的绝对耗时T(draft)、T(verify) 与接受长度 τ
T(draft)草稿模型提出候选token的耗时草稿模型的规模，以及需要草拟多少token；模型越小草拟越快
T(verify)目标模型检查候选token的耗时目标模型的规模，以及每轮校验多少个草稿token
每秒token数≈ 1 / L，用户实际感受到的吞吐单token延迟L
接受长度 τ每次校验平均被接受的草稿token数草稿模仿目标的程度：猜得越准，通过校验的token越多
...
图3：原始投机解码用一个独立的小LLM从零开始猜；EAGLE-3把它换成一个草稿层，复用目标模型的隐藏特征与LM head。
...
图4：扩散对每个位置并行去噪，DFlash把它带入草拟环节。EAGLE-3一次草拟一个token，DFlash一次对一整块MASK token并行去噪，并且每个块只注入一次目标模型的上下文特征。时间按论文的数字走：DFlash的5层草稿跑一遍约等于EAGLE-3的5个单层草拟步，比单独一步慢，但它一次草拟16个token，而EAGLE-3用8步草拟8个token。
扩散式草拟带来两点好处。
...
图5：草拟代价与块大小的关系。EAGLE-3为了速度只保留一层，所以草拟 γ 个token要 γ 次；DFlash一次用五层，任意 γ 下代价都是平的。
...
图6：DFlash把目标模型在所有已验证前缀位置上的特征转成key和value，注入每个草稿层的KV缓存，因此每一层都能看到完整前缀。EAGLE-3只依赖最后一个token，而且只在最底层加入。
...
图7：DSpark增加一个顺序头处理块内依赖，再用一个置信头给每个草稿位置打分；感知负载的调度器在校验之前裁掉低置信度的队列。
...
图8：为了让整块保持连贯，DFlash 2增加一个路径选择器，在相邻位置之间挑出连贯的token序列，并用局部卷积减轻靠近块尾部的接受率衰减。
...
图9：完整的解码竞速。EAGLE-3、DFlash、DSpark三条泳道用的是DeepSpec在Qwen3-8B上测得的接受长度（表2），DFlash 2的泳道用Inco报告的2.7到3.4倍区间。
表2到表4汇总了三个目标模型上报告的接受长度，并给出每个数字背后的公开草稿模型，方便复现。

方法τ相对基线草稿模型

EAGLE-32.66基线deepseek-ai/eagle3_qwen3_8b_ttt7
DFlash3.11+17%deepseek-ai/dflash_qwen3_8b_block7
DSpark3.72+40%deepseek-ai/dspark_qwen3_8b_block7
DFlash 2无无无
...
EAGLE-3无无yuyijiong/Qwen3.5-4B-Eagle3（社区）
DFlash4.92基线z-lab/Qwen3.5-4B-DFlash
DSpark5.49+12%未开源
DFlash 25.97+21%未开源
...
EAGLE-3无无无
DFlash无无kstoyanov99/Qwen3.8-27B-Dflash（社区）
DSpark3.62基线RadixArk/Qwen3.8-27B-DSpark（社区）
DFlash 24.80+33%incoai/Qwen3.8-27B-DFlash2
...
图10：拒绝采样让输出与目标分布保持一致；放宽的接受规则则跟随草稿的偏好。
调度顺序也会决定无损
...
EAGLE-3（2025）GSM8KHumanEvalMT-Bench、AlpacaCNN/Daily Mail（摘要）
DFlash（2026）GSM8K、MATH-500、AIME25HumanEval、MBPP、LiveCodeBenchMT-Bench、Alpaca无
DSpark（2026）GSM8K、MATH-500、AIME25HumanEval、MBPP、LiveCodeBenchMT-Bench、Alpaca、Arena-HardDeepSeek-V4线上流量（只测速度）
DeepSpec测试框架gsm8k、math500、aime25humaneval、mbpp、livecodebenchmt-bench、alpaca、arena-hard-v2无

表5：无损在哪里被测量过。各行是各篇论文实验部分用到的评测数据集。

图12：OpenRouter按任务类型划分的流量。83% 的任务从未被投机解码方法测量过。
那么，在这83% 从未被测量过的领域和任务上，无损还成立吗？
LosslessBench：把评测铺到数学与代码之外
...
图13：DFlash按领域划分的接受长度（左）与由此推算的分布发散度D = 1 − α（右）。接受长度越低，token层面的发散越大。
那么这种发散会不会转化成人能感受到的任务质量损失？这正是图14要检验的。

图14：Qwen3-8B在LosslessBench上开启与不开启投机解码的对比。每条轴独立缩放，因此每个领域的相对差距都可见。
...
图15：LosslessBench日历简报（L101）上的解码竞速。原始模型用时18.7秒，DFlash用时8.9秒。DFlash最快，而坏掉的页面正是它生成的。
图16是创意写作任务上的评测结果：EAGLE-3与DSpark写出了完全相同的故事，原始模型和DFlash则从同一句开头各自走上不同的轨迹，于是留下三个不同的故事可供评判：

图16：同一个竞速换到1000词的创意简报（LosslessBench L073）。原始模型用时15.2秒，DFlash用时8.5秒。

故事指令遵循拉丁语词汇写作风格

原始模型 · 7/10979词。在进入打斗时收尾，接近违反「不得战斗」的规则。正确、克制。感官细节最强，有具名角色。结尾退回到一段泛泛的自由独白。
EAGLE-3 / DSpark · 6/10最好。998词，所有约束都满足。正确、稀疏。作为小说最弱。同一个论点重复三遍。没有具名角色。把政治讲出来，而不是演出来。
DFlash · 7.5/10最差。1092词，超出9%，还生造了一个sacrae bell。不准确，装饰性。结构最好。完整的从黎明到夜晚的弧线，一个有背景故事的配角，结尾意象最强。
...
pip&nbsp;install&nbsp;modal&nbsp;&amp;&amp;&nbsp;modal&nbsp;setup&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;one-time&nbsp;account&nbsp;linkgit&nbsp;clone&nbsp;https://github.com/lilyzhng/2026_NeurIPS_Education&nbsp;&amp;&amp;&nbsp;cd&nbsp;2026_NeurIPS_Education/submission/teaching_materials/labSPEC_MODE=vanilla&nbsp;modal&nbsp;deploy&nbsp;modal_vllm_serve.py&nbsp;&nbsp;&nbsp;#&nbsp;prints&nbsp;your&nbsp;server&nbsp;URL
这个脚本会固定镜像，把模型权重缓存到卷里只下载一次，并把服务暴露在一个公开URL上。用完执行modal app stop neurips-spec-lab释放GPU。
先服务原始模型：
vllm&nbsp;serve&nbsp;Qwen/Qwen3-8B&nbsp;--port&nbsp;8000
发一个提示词并测量生成速度（脚本是measure_decoding_speed.py）。然后把服务换成DeepSpec的DSpark草稿器（在Modal上是SPEC_MODE=dspark modal deploy modal_vllm_serve.py），再跑一遍同样的基准：
vllm&nbsp;serve&nbsp;Qwen/Qwen3-8B&nbsp;--port&nbsp;8000&nbsp;--speculative-config&nbsp;\&nbsp;&nbsp;&#39;{&quot;model&quot;:&nbsp;&quot;deepseek-ai/dspark_qwen3_8b_block7&quot;,&nbsp;&quot;method&quot;:&nbsp;&quot;dspark&quot;,&nbsp;&quot;num_speculative_tokens&quot;:&nbsp;7}&#39;
从吞吐反推接受长度
在单张H100上跑5次，加速比是稳定的（均值 ± 标准差，图17）：

图17：Qwen3-8B在单张H100上的解码吞吐，原始模型对比带DSpark草稿。5次运行的均值 ± 标准差。
vLLM不会直接报告接受长度或单token延迟，两者都来自真实测量：延迟来自上面的吞吐，τ 来自服务器的/metrics计数器（提出5180个草稿token，每轮7个，约等于在2606个生成token上做了740轮校验）。计算过程如下：
L(target)&nbsp;=&nbsp;1&nbsp;/&nbsp;136.3&nbsp;tok/s&nbsp;≈&nbsp;7.3&nbsp;ms&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;latency&nbsp;of&nbsp;the&nbsp;target&nbsp;(vanilla)&nbsp;model,&nbsp;per&nbsp;tokenL(dspark)&nbsp;=&nbsp;1&nbsp;/&nbsp;231.4&nbsp;tok/s&nbsp;≈&nbsp;4.3&nbsp;ms&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;latency&nbsp;with&nbsp;the&nbsp;DSpark&nbsp;draft,&nbsp;per&nbsp;tokenτ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;≈&nbsp;3.5&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;acceptance&nbsp;lengthT(draft)&nbsp;+&nbsp;T(verify)&nbsp;=&nbsp;L(dspark)&nbsp;×&nbsp;τ&nbsp;≈&nbsp;15.1&nbsp;ms&nbsp;&nbsp;#&nbsp;cost&nbsp;of&nbsp;one&nbsp;draft+verify&nbsp;passη&nbsp;=&nbsp;L(target)&nbsp;/&nbsp;L(dspark)&nbsp;≈&nbsp;1.70x&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;speedup
二、自己调接受阈值
SGLang的--speculative-accept-threshold-single出厂值是1.0，此时拒绝采样与目标模型一致。把它调低会让草稿token更容易被接受，用无损换速度。作者在LosslessBench的前端设计任务上测了1.0到0.4这几档（L101，与图15同一个日历弹窗提示词，温度1），每一档都重新生成页面并记录接受长度与解码速度（图18）。在温度0下阈值不起作用：每一档返回的页面都逐字节相同。

图18：LosslessBench前端设计任务（L101，温度1）上的SGLang接受阈值。
...
SPEC_MODE=eagle3&nbsp;modal&nbsp;deploy&nbsp;modal_vllm_serve.pySPEC_MODE=dspark&nbsp;modal&nbsp;deploy&nbsp;modal_vllm_serve.pymodal&nbsp;run&nbsp;modal_dflash_offline.py&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;DFlash,&nbsp;see&nbsp;note&nbsp;belowpython3&nbsp;build_race_demo.py&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;assembles&nbsp;the&nbsp;demo&nbsp;from&nbsp;your&nbsp;outputs
这几条命令复现的就是第2.3节里的两个实时演示（图15与图16）。
注意：在稳定版0.28.0里vllm serve跑DFlash会崩，只有离线的LLM() 路径进了CI，所以DFlash走modal_dflash_offline.py。它用的草稿器也是z-lab的发布版，而不是DeepSpec的。
四、在自己的服务器上跑LosslessBench
用LosslessBench的L101任务：「一个出色的、能平滑融入界面的半透明日历弹窗」。
python3&nbsp;generate_frontend_task.py&nbsp;--url&nbsp;&lt;your-url&gt;&nbsp;--label&nbsp;vanilla
因为用的是贪心解码，无损给出了一个很具体的预测：带草稿器的版本应该逐字节复现原始模型的HTML。而实际测到的结果并不是这样。
在原始服务器和DSpark服务器上各跑一次，然后比较两个HTML文件：

对比相同前缀首次分歧最终长度

原始模型vs DSpark8299字符（页面的76%）`transition: color 0.2s` 变成 `0.3s`10924对10535字符
...
领域原始模型DSparkEAGLE-3DFlash

代码138.1311.9（2.3倍）158.8（1.15倍）311.3（2.25倍）
创意138.2416.7（3.0倍）229.5（1.66倍）265.6（1.92倍）
前端137.6333.1（2.4倍）208.2（1.51倍）274.0（1.99倍）
...
图A2：同一个模型、同一个前端提示词，左为原始模型，右为处于厂商组装的加速栈下。这里的元凶是量化。
推理加速的归属正在上移
...
图A3：模型层一步步吸收加速能力，留给服务层的位置收缩到一件事：把模型服务好。改绘自LosslessBench的图6，把边界画成台阶。
...
α&nbsp;=&nbsp;1&nbsp;−&nbsp;E[D(p,&nbsp;q)]
这里的p和q分别是目标模型和草稿模型的下一个token分布，D(p, q) 是两者之间的总变差距离。同一套分析也从接受率推出了表1里定义的接受长度 τ：
τ&nbsp;=&nbsp;(1&nbsp;−&nbsp;α^(γ+1))&nbsp;/&nbsp;(1&nbsp;−&nbsp;α)
其中 γ 是每个校验周期里草稿token的数量。
...
T(verify)&nbsp;=&nbsp;4.3&nbsp;ms&nbsp;&nbsp;&nbsp;#&nbsp;one&nbsp;target&nbsp;forward&nbsp;pass&nbsp;(Qwen3-8B&nbsp;at&nbsp;230&nbsp;tok/s&nbsp;≈&nbsp;4.3&nbsp;ms/token)T(draft)&nbsp;&nbsp;=&nbsp;1.3&nbsp;ms&nbsp;&nbsp;&nbsp;#&nbsp;drafting&nbsp;costτ&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=&nbsp;3&nbsp;tokens&nbsp;#&nbsp;accepted&nbsp;per&nbsp;verification&nbsp;passL&nbsp;=&nbsp;(1.3&nbsp;ms&nbsp;+&nbsp;4.3&nbsp;ms)&nbsp;/&nbsp;3&nbsp;≈&nbsp;1.9&nbsp;ms&nbsp;per&nbsp;token&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;per-token&nbsp;latencyη&nbsp;=&nbsp;4.3&nbsp;ms&nbsp;/&nbsp;1.9&nbsp;ms&nbsp;≈&nbsp;2.3x&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;#&nbsp;speedup:&nbsp;2.3x
总结起来，投机解码的速度取决于三个因素：草拟时间、校验时间、接受长度。 接下来看四代架构如何依次改进这三个决定因素。

结语

投机解码的加速红利，最终取决于你愿意为无损付出多少代价。 四代架构拆的是同一组变量的三个方向：EAGLE-3把接受长度从2.66拉到3.72，DFlash把草拟从串行改成一次并行去噪，DSpark把校验裁剪到真正值得校验的位置，DFlash 2再用200万参数的路径选择器把接受长度抬到5.97。对做LLM服务的人来说，最该记住的是LosslessBench那个数字：被测试过的领域只占token用量的17%。默认阈值、默认调度器、默认量化叠在一起，无损很可能只在你测过的那部分任务上成立。上线之前，先在自己的业务文本上量一次接受长度和任务正确率，比抄任何一个SOTA数字都管用。
