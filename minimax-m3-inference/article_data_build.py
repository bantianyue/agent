...
DATA = {
  "summary": [
    {
      "key": "核心思路",
      "body": "从完整的推理路径入手，四大优化叠加：在线量化加速注意力 GEMM、稀疏注意力+FP8 KV缓存降带宽、EAGLE3投机解码降目标前向、ATOMesh 支持 P/D 分离。"
    },
    {
      "key": "关键收益",
      "body": "EAGLE3 整体草稿接受率 73.45%、首个 token 85.8%；在 MI355X 上 FP4 模式每 GPU 吞吐对标/超越公开发布的 InferenceX 数据。"
    },
    {
      "key": "目标瓶颈",
      "body": "四大优化共同解决注意力 GEMM 开销、KV 缓存带宽、稀疏注意力调度与目标模型前向频率这四个服务瓶颈。"
    }
  ],
  "lead": [
  ...
  "sections": [
    ...
    },
    {
      ...
      "figs": [
        {
          "src": "fig01.png",
          "caption": "图 1：MiniMax-M3 在线量化路径——选中 GEMM 权重加载时由 1x32 块缩放转为 PTPC FP8。"
        }
      ]
    },
    {
      "type": "h2",
      "title": "稀疏注意力与 FP8 KV 缓存",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig02.png",
          "caption": "图 2：MiniMax-M3 KV 缓存布局——q_head=64、kv_head=4 组织为 4 组 KV 头稀疏注意力（每组 16 query / 1 KV head）。"
        }
      ]
    },
    {
      ...
      "figs": [
        {
          "src": "fig03.png",
          "caption": "图 3：MiniMax-M3 EAGLE3 融合草稿模型执行与分布式 argmax——token 经本地嵌入、4 卡 TP 注意力/MLP、所有结果经分布式 argmax 输出，降低投机解码开销。"
        }
      ]
    },
    {
      ...
      "figs": [
        {
          "src": "fig04.png",
          "caption": "图 4：基于 Mooncake RDMA KV 与索引器-键缓存传输的 ATOMesh 上 MiniMax-M3 P/D 分离。"
        }
      ]
    },
    {
      ...
      "figs": [
        {
          "src": "fig05.png",
          "caption": "图 5：搭载 ATOM 的 MI355X 上 MiniMax-M3 FP4 8K/1K 每 GPU 服务吞吐与已发布 InferenceX 结果对比。"
        },
        {
          "src": "fig06.png",
          "caption": "图 6：搭载 ATOM + M3 EAGLE 的 MI355X 上 FP4 8K/1K 每 GPU 吞吐与 InferenceX 结果对比。"
        },
        {
          "src": "fig07.png",
          "caption": "图 7：搭载 Mooncake ATOMesh 的 MI355X 上 FP4 8K/1K 每 GPU 吞吐与 InferenceX 结果对比。"
        }
      ]
    },
    {
