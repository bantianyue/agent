...
t(s,"以上都在模型权重之内;PagedAttention 与 RadixAttention 活在 serving 引擎,是同一压力在另一层的回答。请求到来时引擎要给它分配 KV 的 GPU 内存;朴素做法是给"最大可能 seq 长度"预留连续内存——允许最多 4096 的请求,g提前占 4096 槽,不管它是用了 40 还是 4000。这浪费约 60-80% GPU 内存,请求间碎片雪上加霜。PagedAttention(vLLM 用)把 KV cache 管得像 OS 管虚拟内存:按需分配定长块、块表把每请求的逻辑块映射到任意空闲物理块——无预分配、无碎片,浪费降到 4% 以下。RadixAttention(SGLang 用)更进一步:多个请求共享公共前缀(如发给所有用户的 long system prompt)时,前缀的 KV 块只需算一次;它把 KV 块按 token 序列存进一棵 radix tree,新请求沿树走、找最长匹配前缀、复用那些块、只算真正新的后缀。多轮负载下 RadixAttention 打到 75-95% 命中率;一份 system prompt 服务数千用户只算一次、按 LRU 策略到被逐出。二者都不改模型用的 attention 机制:Llama 3 GQA 在 vLLM 或 SGLang 下都正常;两层相互独立。")
s=h2("收束：那条贯穿全篇的线")
