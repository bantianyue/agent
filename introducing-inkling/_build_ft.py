...
blocks=[json.loads(l) for l in open(f"{ART}/blocks.jsonl") if l.strip()]
src=[b['content'] for b in blocks if b['type']=='text']
assert len(src)==88, len(src)
...
assert len(ZH)==88, len(ZH)

def onepara(s):
    return re.sub(r'\n\s*\n+', '\n', s.strip())

out = "\n\n".join(onepara(z) for z in ZH)
open(f"{ART}/full_translation.md","w",encoding="utf-8").write(out+"\n")
paras=[p.strip() for p in re.split(r'\n\s*\n', out) if p.strip()]
print("paragraphs:", len(paras))
for k,v in {4:"为了展示定制",19:"Inkling的设计是广博",25:"Inkling一次性构建",28:"Inkling在Design Arena",32:"Create a premium",34:"Inkling通过一个由GPT Codex",38:"Inkling支持可控的思考力度",72:"我们依靠大规模异步RL",74:"我们还观察到在RL训练过程中"}.items():
    print(k, v in paras[k])
