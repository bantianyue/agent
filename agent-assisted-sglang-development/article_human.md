...
三个常用skill：`debug-cuda-crash`（CUDA crash调试）、`sglang-diffusion-benchmark-profile`（diffusion基准测试和profile）、`llm-torch-profiler-analysis`（LLM profiler分析）。
...
| 容量规划 | llm-serving-capacity-planner | 解析启动日志，解释权重内存、KV cache 预算、CUDA graph 开销、请求容量和 OOM 压力 |
| Trace 分类 | llm-torch-profiler-analysis | 生成固定的 kernel、overlap 机会、融合模式三张表，将 kernel 映射回 Python 源码；同一工作流也存在于 AI-Infra 中供跨框架使用 |
...
| SGLang 审查 / PR 历史 | sglang-humanize-review 和 model-pr-history-knowledge | 对照真实维护者讨论模式审查 SGLang patch，保持 PR 驱动的模型演化历史靠近变更的源码 |
| SGLang SOTA 性能循环 (Loop Engineering) | sglang-sota-humanize-loop | 首先公平对比 SGLang 与目标开源推理框架，然后将差距判断、profiling、打补丁和重新验证放入 Humanize/RLCR 循环 |
...
| 案例 | 结果 | 关键点 |
|------|------|--------|
| Router long-context tokenization 去重，PR #28744 | DeepSeek-V4-Flash 上 60k/125k token 请求的 idle TTFT 下降约 29%/41%；60k token 负载下 TTFT 下降 34%-49% | Agent 处理了 cache-aware routing、chat-encoder 对齐、engine 侧的 input_ids 回退和 proxy body 构造，避免了 router 和 engine 中的重复 tokenization |
...
| Cohere2Moe NVFP4 fused-MoE 路径，PR #27401 | 1x B300 上 chat 吞吐 +26%，summarization +21%，超越另一开源推理框架 +4.1%/+6.8% | 补全了路由语义，使现有的 flashinfer_trtllm NVFP4 fused-MoE kernel 能在真实模型路径中正确使用，通过 GSM8K/MMLU 检查 |
| Kimi Delta Attention CuteDSL prefill kernel on SM100，PR #27488 | B200 上 Delta Attention prefill 比 Triton 快 1.08x-1.52x；GSM8K 从 0.915 提升到 0.920，新增了针对真实 gate 幅度的回归测试 | 这个 kernel 任务覆盖了模型的 gate 分布、数值溢出、host 开销、真实模型精度和单元测试 |
| Spectral Progressive Diffusion，PR #27524 | FLUX.1/FLUX.2/Z-Image/Wan/Qwen-Image 去噪加速分别达到 1.63x/1.77x/2.07x/2.32x/1.6x | Diffusion 端的系统优化：早期去噪在较低潜空间分辨率运行，然后 GPU DCT 上采样在高频细节重要时恢复全分辨率 |
...
完整的SGLang SOTA Performance Loop包含以下阶段：固定公平基准测试 → 差距判断 → profiling → 流水线分析 → 打补丁 → 重新验证。
...
<strong style="font-size:16px;color:#1a6ba0;">4.3 Codex Goal：更低成本的循环实现</strong>
...
| 状态位置 | Plan、prompt、summary、review 结果在 `.humanize/rlcr/...` 下 | 当前 Goal 线程加上 `artifact_root` 下的 manifest/evidence |
| 审查方法 | 停止钩子、Codex Review 以及 git/state/schema 检查 | Goal 级自检、artifact 契约和人工抽查 |
| 成本 | 双模型角色参与，每轮成本更高 | 一个 Goal 承担执行和检查，降低成本 |
| 主要风险 | 循环设置更复杂，每轮等待时间更长 | Goal drift 或过早完成，除非有明确的硬停条件 |

Goal版本保留了相同的benchmark、profile、accuracy和artifact要求。主要区别是将执行和审查融合为一个持久目标。有了明确的硬停条件，它可以用更少的编排完成同样的SGLang SOTA Performance Loop。
...
KDA-Pilot的策略是将内核优化分离为独立的任务，让agent不能自由修改整个SGLang仓库。
...
| #29361 | LTX-2.3 residual-gate update | 大 B200 LTX-2.3 行比现有 Triton 路径提升 1.108x-1.130x，相邻 diffusion 行最高达 2.587x | LTX-2.3 HQ T2V，E2E 时间从 46644.08ms 降至 45198.37ms（1.032x） |
...
<strong>Define the task boundary before starting the agent.</strong>
「Optimize SGLang」太宽泛。「Match another open-source inference framework for `Qwen/Qwen3-Next-80B-A3B-Instruct-FP8` on 2x B200 under fixed `1000->1000` and `8000->1000` workloads」才是可执行的目标。
