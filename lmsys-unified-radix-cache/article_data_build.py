...
DATA = {
  ...
  "summary": [
    {
      "key": "设计核心",
      "body": "把『共享前缀身份』与『组件特定复用有效性』解耦：一个 token 键控的 radix 拓扑作为每个前缀的规范坐标，全注意力 KV、滑动窗口 KV、Mamba 检查点作为组件挂载其上。"
    },
    {
      "key": "组件语义",
      "body": "FULL 提供路径复用、SWA 提供窗口复用、MAMBA 提供检查点复用。组件通过 hooks 控制匹配/分裂/插入/加锁/驱逐，新混合组合无需新缓存树。"
    },
    {
      "key": "实测收益",
      "body": "L3 外部 tier 让后期回合命中率保持在 DeepSeek-V4 98%、Inkling 96.8%；session-aware 驱逐在 SWE-bench 上降低 2.9%-16.6% TTFT；实验性 Rust 树核在滑动窗口负载上降低最多 42% TTFT。"
    }
  ],
  "sections": [
    {
      ...
      "fig_after": {
        "2": [
          {
            "src": "fig01.png",
            "caption": "图 1：一个 radix 拓扑提供规范的前缀身份。FULL、SWA、MAMBA 组件分别强制路径、尾部窗口、检查点复用语义；HiCache 控制它们的载荷放在 GPU L1、Host L2 还是外部 L3；sidecar 跟随声明的源池而不改变树行为。"
          }
        ]
      }
    },
    ...
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig02.png",
            "caption": "图 2：组件投票把遍历深度与可复用前缀深度分开。行走到达 n4，而 n2 才是 FULL、SWA、MAMBA 都接受的最深可复用边界。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig03.png",
            "caption": "图 3：组件可在独立索引空间间翻译，而每个 sidecar 精确复用其声明源池的索引。两种关系都跨 HiCache tier 保持同一共享前缀身份。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig04.png",
            "caption": "图 4：HiCache 多轮基准。L1 和 L1+L2 在对话历史增长时撞上容量极限，命中率下滑、TTFT 上升；外部 L3 tier 保住可复用前缀更久，给出最高的有效输入 token 吞吐。两行用不同模型/GPU/请求规模，只可在同一行内比较 tier。"
          }
        ]
      }
    },
    {
      "type": "h2",
      "title": "session-aware 驱逐：让活跃会话不被误伤",
      "paras": [
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig05.png",
            "caption": "图 5：三个活跃会话共享一棵 radix 拓扑。FULL 跟踪引用前缀路径、SWA 跟踪尾部窗口、MAMBA 跟踪可复用边界；GPU 与主机驱逐优先扫未引用条目，引用条目作为后备仍可驱逐。关闭会话只移除保留信号而不立即删除可复用缓存。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig06.png",
            "caption": "图 6：SWE-bench 命中率与 TTFT。上方面板堆叠缓存驻留（基准与 Unified 配置的设备命中 + 主机命中）；下方面板是实测 TTFT，顶部标注相对基线的降幅。两模型用独立 TTFT 刻度。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig07.png",
            "caption": "图 7：实验性 Rust 与 Python 树核在 FULL、SWA、混合 SSM 模型上的 TTFT。上排是总 TTFT 与 CUDA-event 计时的 GPU prefill 区间，下排是二者差值。三个模型用独立 y 刻度。"
          }
        ]
      }
    },
