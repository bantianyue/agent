...
DATA = {
  "summary": [
    {
      "key": "核心发现",
      "body": "逐位一致的训练/推理内核确实能消除 logprob 差距并防止其随窗口扩大爆炸，但在多数工作负载下并未转化为奖励或准确率增益，而代价是 2–5× 训练吞吐。"
    },
    {
      "key": "方法创新",
      "body": "用统一模型定义 + 批不变内核（含线性注意力切到循环内核）实现训练器与生成器逐位一致，本质上是把它当作调试工具。"
    },
    {
      "key": "适用场景",
      "body": "仅 math@offpolicy=12 与终端智能体上略有效益；搜索类与多数窗口下两者在噪声中难分高下。"
    }
  ],
  "lead": [
  ...
  "sections": [
    {
      ...
      "figs": [
        {
          "src": "fig01.png",
          "caption": "图 1：训练/推理 logprob 绝对差（200 步）：vLLM 原生最高且最尖，TorchTitan 无 BI 居中，批不变模型全程最低，step 0 处 BI 曲线恰好为 0（圆圈标示）。"
        },
        {
          "src": "fig02.png",
          "caption": "图 2：rollout 平均训练奖励：带 BI 的 TorchTitan 模型最高（约 0.7），无 BI 略低，vLLM 原生最低（约 0.6）。"
        }
      ]
    },
    {
      "type": "h2",
      "title": "背景：确定性、批次不变性与逐位一致性",
      "paras": [
    ...
    },
    {
      ...
      "figs": [
        {
          "src": "fig03.png",
          "caption": "图 3：Qwen3.5 批不变模式数据流。左训练器：训练 token 经循环前向得到激活/logits 与损失，反向用分块内核产梯度；右推理/生成器：提示 token 经循环预填充进入循环状态，循环解码读取并更新该状态产出下一 token。虚线连接训练器循环前向与生成器预填充/解码，标示两侧逐位一致的算子。"
        }
      ]
    },
    {
      "type": "h2",
      "title": "结果：零失配的异步 RL，性能还是幻觉",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig04.png",
          "caption": "图 4：异步 RL 时间线：rollout 持续解码，暂停时加载新权重并立即在现有 KV 缓存上恢复解码，无需重算。横跨暂停的序列（s5/s7/s6）早期 token 由旧权重生成、后期由新权重生成。"
        }
      ]
    },
    {
      "type": "h3",
      "title": "4.2.1 MATH：单轮、长生成",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig05.png",
          "caption": "图 5：offpolicy=12 时三组运行的验证奖励总和：带 BI 在 step 170 达最高峰（约 17），多数时间持平或高于另两者，vLLM 原生早期落后。"
        }
      ]
    },
    {
      "type": "h3",
      "title": "窗口扫描：BI 稳定 logprob 差但不提升准确率",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig06.png",
          "caption": "图 6：offpolicy=4 的 logprob 绝对差：无 BI（绿）全程略高于 BI（橙）且尖峰更高，两者从约 0.004 漂到 0.006。"
        },
        {
          "src": "fig07.png",
          "caption": "图 7：offpolicy=12 的 logprob 绝对差：无 BI（棕）升至约 0.011，BI（灰）维持约 0.009，step 100 后差距拉开。"
        },
        {
          "src": "fig08.png",
          "caption": "图 8：offpolicy=32 的 logprob 绝对差：step 100 后无 BI（紫）失控至约 0.065，BI（粉）仅约 0.035，差距爆炸。"
        },
        {
          "src": "fig09.png",
          "caption": "图 9：offpolicy=4 的验证奖励：BI（橙）与无 BI（绿）全程交织，都停在约 0.43–0.47，无分离。"
        },
        {
          "src": "fig10.png",
          "caption": "图 10：offpolicy=12 的验证奖励：BI（灰）在 step 70–170 多数时间在上方、峰值约 0.57，但反复交叉，最终无 BI 更高。"
        },
        {
          "src": "fig11.png",
          "caption": "图 11：offpolicy=32 的验证奖励：BI（粉）与无 BI（紫）在 0–0.4 间大幅震荡，全程无分离，天花板低于窄窗口。"
        },
        {
          "src": "fig12.png",
          "caption": "图 12：offpolicy=4 的训练器吞吐：无 BI 统一模型（绿）最高（约 11k–15k）且衰减，vLLM（蓝）居中，BI（橙）最低且剧烈尖峰。"
        },
        {
          "src": "fig13.png",
          "caption": "图 13：offpolicy=12 的训练器吞吐：无 BI（棕）最高且平（约 14.5k），vLLM 原生（红）约 12.2k，BI（灰）约 8.4k 带少量下凹。"
        }
      ]
    },
    {
      "type": "h3",
      "title": "MoE：差距下降来自统一模型，而非 BI",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig14.png",
          "caption": "图 14：Qwen3.5-35B-A3B 的 logprob 绝对差：vLLM 原生（绿）全程最高（约 0.017–0.02），统一模型无 BI（蓝）与带 BI（紫）在约 0.014 处重叠。"
        },
        {
          "src": "fig15.png",
          "caption": "图 15：Qwen3.5-35B-A3B 的 rollout 平均训练奖励：三组全程交织，从约 0.6 共同升至约 0.8，"
        }
      ]
    },
    {
      "type": "h3",
      "title": "4.2.2 Search-R1：多轮、短生成",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig16.png",
          "caption": "图 16：Search-R1 的 logprob 绝对差：两臂从 0 开始、前几步尖峰超 0.03 后衰减，最后 100 步 BI（橙）稳于约 0.002、无 BI（紫）约 0.005。"
        },
        {
          "src": "fig17.png",
          "caption": "图 17：Search-R1 训练精确匹配奖励：BI（橙）与无 BI（紫）step 100 都升至约 0.6 后保持交织。"
        },
        {
          "src": "fig18.png",
          "caption": "图 18：Search-R1 验证精确匹配奖励：step 50 后两臂在约 0.49–0.56 间完全重合。"
        },
        {
          "src": "fig19.png",
          "caption": "图 19：Search-R1 训练器吞吐：无 BI（紫）全程远高于 BI（橙），两者都因多轮 step 代价变化而尖峰。"
        }
      ]
    },
    {
      "type": "h3",
      "title": "4.2.3 TMax：多轮、长生成（终端智能体）",
      "paras": [
      ...
      "figs": [
        {
          "src": "fig20.png",
          "caption": "图 20：TMax 的 logprob 绝对差：BI（绿）step 0 恰为 0 后与统一无 BI（黄）紧密交织，均约 0.004–0.006、无分离。"
        },
        {
          "src": "fig21.png",
          "caption": "图 21：TMax 二值任务奖励：BI（青）与无 BI（橙）前 50 步纠缠，BI 平滑曲线在 step 55–80 略高，终值约 0.65。"
        },
        {
          "src": "fig22.png",
          "caption": "图 22：TMax 训练器吞吐：无 BI（橙）约 7.5k–10k，BI（青）全程约 1.5k–2k，约 5 倍差距。"
        }
      ]
    },
    {
      "type": "h2",
      "title": "结论：一个调试工具，而非生产默认",
      "paras": [
