...
PyTorch 2.0的编译生态像一座冰山：你看到的是 `torch.compile` 一行代码带来的加速，但水下是Dynamo（图捕获）、AOTAutograd（自动微分）、Inductor（代码生成）等一整套体系。
...
**FX Graph将Python代码展平成一系列节点。** 原本嵌套的 `torch.relu(self.linear(x))` 被拆成了三步：placeholder（输入）→ call_module（调用linear）→ call_function（调用relu）→ output（输出）。
