...
DATA = {
  ...
  "summary": [
    {
      "key": "系统设计",
      "body": "冻结的大基座 + 一组可注册的 LoRA 专家，通过 MoL Proxy 按每个用户回合选一个专家；基座不参与再训练，新能力通过增训与注册新适配器获得。"
    },
    {
      "key": "两大支柱",
      "body": "适应（Adaptation）靠版本化的模型—harness 对递归自改进；协作（Collaboration）靠 Mixture-of-LoRA 在冻结基座上组合各专家能力。"
    },
    {
      "key": "路由代价",
      "body": "路由由 L0 在 24-token 预算内完成，Venti 路由准确率 99.12%，路由+摘要两跳占三跳总耗时的约 32%，且不损害任务质量。"
    }
  ],
  "sections": [
    {
      ...
      "fig_after": {
        "2": [
          {
            "src": "fig02.png",
            "caption": "图 2：扩展模型能力的三种方式。MoE 扩展基座本身；skills 在固定模型外加脚手架；MoL 保持基座冻结，通过 Proxy 中介的路由循环组合专家 LoRA 适配器。这一分离是实现持续学习与集体智能目标的基底。"
          }
        ]
      }
    },
    ...
    {
      "type": "h3",
      "title": "own-view 让路由变得廉价，KV 复用两级",
      "paras": [
    ...
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig03.png",
            "caption": "图 3：生成式 UI 的三种做法。HTML-native 最大化表达力但继承原始 web 开发的所有问题；Verifiable-schema 易于验证但限制表达；UI4A（component-native）让模型在运行时边界内编写普通前端代码，既恢复表达力又无 schema 天花板。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig04.png",
            "caption": "图 4：UI4A-Bench 的 gallery 切片。每张卡片作为运行时边界约束下的普通前端代码生成，按编译/渲染正确性、控制接线与屏幕适配计分。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "1": [
          {
            "src": "fig05.png",
            "caption": "图 5：可执行组合（executable composition）。REPL harness 让模型把可执行构造与验证复用作为与工具交互的接口。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig06.png",
            "caption": "图 6：共享一个 harness 的三条循环。MindForge 的任务发现、轨迹扩展与模型/配置更新，连同版本化模型—harness 对的递归改进，构成 Macaron 的适应闭环。"
          }
        ]
      }
    },
    {
      "type": "h2",
      "title": "基础设施：MinT、LongStraw 与稀疏基座",
      "paras": [
    ...
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig07.png",
            "caption": "图 7：LivingBench——在演变条件下评估有状态的模拟个人生活助理，衡量智能体长期服务同一用户的能力。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig08.png",
            "caption": "图 8：UI4A-Bench 评估循环。每个用例将请求与相关用户上下文结合，在组件边界内评估生成的 UI。"
          }
        ]
      }
    },
    {
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig01.png",
            "caption": "图 1：Venti 主要结果概览。"
          }
        ],
        "1": [
          {
            "src": "fig10.png",
            "caption": "图 10：Macaron-V1（Venti）与六个对比模型在十二条基准行上的点估计（0–100 归一）。跨行指标与协议不同，所以表格不标 '胜者'，只做同协议行内的公平比较。"
          }
        ],
        "2": [
          {
            "src": "fig09.png",
            "caption": "图 9：Tall 在七条同时评测的两系统基准上与 Venti 的逐行对比。"
          }
        ]
      }
    },
