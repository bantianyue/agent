...
t(s,"所以跑一次模型,本质是 CPU 排一排 kernel launch、GPU 一条条消化;用 profiler 看 GPU 时间线,就是数百个小方框,每个都是一个 kernel。")
t(s,"每次 launch 有几微秒固定开销,累积起来相当可观。")
s["fig_after"]={"1":[{"src":"fig01.png","caption":"图1：许多个小方框,每个都是一个 kernel。"}]}
...
s["fig_after"]={"0":[{"src":"fig02.png","caption":"图2：Claude 生成的 GPU 结构示意图,数字取自 H100。"}]}
...
d={"title":"到底什么是 GPU kernel,以及为什么值得自己写",
 ...
 "lead":["这段时间满屏都在聊推理与性能工程,每个人都似乎随口一句 kernel、custom kernels。这篇用不到两千字的篇幅说清三件事:GPU kernel 到底是个什么、它怎么被调起来、以及为什么(在别的手段之后)值得自己写一个。"],
 "sections":S,
