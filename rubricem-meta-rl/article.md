...
论文的核心视角非常清晰：评分标准（rubric）不应只是裁判在最终答案上打分的工具，而应成为**贯穿强化学习全过程的共享接口**。
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
RubricEM最让我印象深刻的一点是它对"共享接口"这一设计哲学的坚持。很多强化学习工作会把架构设计、信用分配和记忆机制当作三个独立问题分别处理：用不同的模型、不同的目标和不同的数据结构。RubricEM让同一个评分标准贯穿驱动规划、裁判评判和经验蒸馏，三件事用同一套语言体系，减少了接口摩擦。
<br><br>
另一个值得关注的点是"文本空间学习"的思路。传统RL只在参数空间做梯度更新，但语言模型的独特之处在于它的表达能力可以直接产生自然语言文本。RubricEM的反思元策略把梯度更新和文本更新并行起来，实际上是在参数和语言两个空间同时"记住"经验。
<br><br>
