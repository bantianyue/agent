...
![](figure1_chunked.png)
<span style="font-size:12px;color:rgb(153,153,153);">RubricEM整体框架概览：评分标准引导的Agent推理轨迹分解、阶段结构化GRPO（SS-GRPO）的信用分配、以及共享骨干网络的反思元策略训练。评分标准作为共享接口贯穿Agent、裁判和记忆演化。</span>
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
RubricEM最让我印象深刻的是它对"共享接口"这一设计哲学的坚持。很多强化学习工作会把架构设计、信用分配和记忆机制当作三个独立问题分别处理：用不同的模型、不同的目标和不同的数据结构。RubricEM让同一个评分标准贯穿驱动规划、裁判评判和经验蒸馏，三件事用同一套语言体系，减少了接口摩擦。<br><br>
另一个值得关注的点是"文本空间学习"的思路。传统RL只在参数空间做梯度更新，但语言模型的独特之处在于它的表达能力可以直接产生自然语言文本。RubricEM的反思元策略把梯度更新和文本更新并行起来，实际上是在参数和语言两个空间同时"记住"经验。<br><br>
当然，这套框架是否真的能推广到真实网页搜索、不同模型规模和更广泛的深度研究场景，还需要更多验证。但从方法论上看，它为"如何训练不依赖可验证奖励的深度研究Agent"提供了一个清晰、完整且经验证实的回答。
</div>
</div>
