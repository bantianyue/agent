...
data={"title":"一个 MoE token 被路由到专家时,到底发生了什么(8 步原子拆解)",
 ...
 "summary":[
   {"key":"一句话","body":"社区科普作者 作者用 8 步把『token 过 MoE 层』拆到原子级:来的不是词而是 hidden 向量→router 逐专家打分→不同 token 选中不同专家→运行时 permute 按专家分组做 grouped GEMM→跨卡时变专家并行+all-to-all→输出逆排列送回→按 router 分数加权合成一个 hidden state。全推约 690 词+1 架构图。"},
   ...
   {"key":"来源与性质","body":"普通长推 +1 图,非厂商口径;文内数字为教学性示意值(top-2 打分与 GPU 分配表),非实测基准。与 MoE 工业实现细节(负载均衡、aux loss、capacity factor、token drop)不冲突,属直觉入门。",
   }],
 "lead":[
   "面试被问『MoE 怎么加快推理』时,标准答案常是稀疏激活——但很少有人把『一个 token 被路由到 expert 时到底发生什么』讲到原子级。这条长帖正好补上:从向量打分、permute 分组、跨卡 all-to-all,到 grouped GEMM、结果归还与加权合并,8 步拆得明明白白。",
   "本条为作者的一则 ~690 词长文+自代架构图的全文中译;编译保留了 Top-2 打分、按专家分组与 GPU=专家 等全部示意表,配图为原文自带的架构图(转 PNG)。",
 ],
 "sections":sections,
 "conclusion":[
   ...
   "提醒读者:里头的 0.71/0.52 之类是作者的示意数值,用来讲权重为何是权重;真实 router 给分、Top-k 选取、capacity/drop 与负载均衡的处理要复杂数档。把它当直觉入门,别当成实现规格。原文可回源帖查看讨论。",
 ],
}
