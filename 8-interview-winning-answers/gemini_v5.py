...
async def main():
    async with websockets.connect(PAGE_WS, max_size=20*1024*1024) as ws:
        ...
        # First check current URL and title
        ...
        for i in range(40):
            ...
            large = [x for x in imgs if x['w'] >= 300]
            if large:
                best = max(large, key=lambda x: x['w']*x['h'])
                print(f'LARGE IMAGE at {(i+1)*5}s: {best["w"]}x{best["h"]}')
                ...
                if du.startswith('data:'):
                    ...
                    print('Canvas returned non-data URL')
                return
            
            if i % 4 == 0:
                sizes = [f'{x["w"]}x{x["h"]}' for x in imgs[:3]]
                print(f'  {(i+1)*5}s: still waiting ({len(imgs)} small: {", ".join(sizes)})')
