...
DATA = {
    "title": "AI 采用从真实开始：语义层与可靠数据层如何解锁 Agent 企业化",
    "summary": [
        {
            "key": "核心论断",
            "body": "AI 采用受制于信任：用户被错误答案伤害后会绕开系统；解决方案是语义层 + 运营事实层"
        },
        {
            "key": "四机制",
            "body": "git 知识层、人工合并、发言前验证、全 Agent 共享一个审核语料库"
        },
        {
            "key": "结果",
            "body": "内部数据 Agent 每周 1000+个仓库支撑的问题，信任通过口碑传播"
        }
    ],
    "lead": [
    ...
    "conclusion": [
        "要明确这是什么：它不是一个新的 Agent（智能体）框架，因为我们构建在已有基础之上；不是「语义层（semantic layer）」，那只是底层基础；也不是一个已经完成的、自主运行的系统——我们不会假装它是。它是一个尚处早期、但飞轮已经转起来的构建：每一次调查、每一次修复，都让它比发现它时更聪明一点；每一条修正，都是记录里又多一行经过审查的内容。",
        "这个艰难而不光鲜的问题，是让 Agent（智能体）对底层不断变化的数据保持诚实。这正是我们在解决的问题。如果这也是你的问题，来和我们一起构建吧。"
    ],
    "reference_url": "https://replit.com/blog/ai-adoption"
}
