...
在 PyTorch 2.0 的编译栈里，FX Graph 经常被跳过——大家都追 torch.compile、Dynamo、Inductor 这些更耀眼的名字，很少有人先停下来搞懂这层 IR。**但事实是，从 Dynamo 到 Inductor，每个组件都在以某种方式操作 FX Graph。它是整个编译生态的底层语言。**

本文从 DAG 的概念开始，深入到 Graph、Node、GraphModule 三个核心对象，最后拆解 symbolic tracing 的拦截机制和它的硬边界。作为入门篇，目标是让你后续读 torch.compile 源码时不至于卡在第一关。
...
**在 FX Graph 的语境下，DAG 意味着每段 Python 代码都被拍平成一个节点序列，节点之间有明确的数据流向。** 没有循环，没有分支条件，只有节点和边。
...
**`node.args`** 是当前节点依赖的节点元组，**通过 args 就能追溯完整的数据流向。**
...
**`node.target` 最容易让人误解**——含义完全取决于 `node.op`。不能孤立地读 target。
...
普通 Python 代码运行时立即执行，执行完你有结果，但没有计算图——不知道是怎么算出来的。Symbolic tracing 要解决的就是这个问题。
...
**不执行任何实际计算，只模拟和记录。** 听起来巧妙，但这种方式有明确的硬边界。
...
有了 FX Graph 的基础，下一步就是 TorchDynamo。下个系列会讲 TorchDynamo——字节码级别的 JIT 编译器，看它怎么用不同的方式处理相同的 tracing 问题。
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
这篇是 Jino Rohit「Torch Internals」系列的第一篇，也是整个系列的基础。它的核心是把 FX Graph 从"torch 编译器里的黑盒子"还原成三个你可以手写操作的对象——Graph、Node、GraphModule。想理解 torch.compile 体系，这一课值得先花时间吃透。<br><br>
实际上，symbolic tracing 的三大限制已经暗示了 PyTorch 为什么需要 TorchDynamo 这个字节码级的方案。FX Graph 的"一次建图，全局可用"虽然高效，但因为 Proxy 无法处理动态控制流，它在实际模型上覆盖率有限。理解这些限制比只懂原理更有用——遇到 trace 失败报错时，你能快速判断是框架问题还是模型结构问题。
</div>
</div>
