...
t(s,"DCP 计算路径被仔细优化以绕开 NCCL：用对称内存缓冲让对端 GPU 直接 load/store；query 直接多播进 attention kernel 消费的缓冲；每张 GPU 把 partial attention 输出与 log-sum-exp 统计直接写进对端接收槽，各 rank 用 online softmax 本地合并；还把这些 GPU 间直写与计算融进同一 kernel(s)，比默认 DCP8 每层省约 13% 延迟。")
t(s,"更大的 scale-up 域会改最优策略：NVL72 级系统上宽 EP+DP(DEP) 常比 DCP 更会扩展、同一 decode SLO 下吞吐更高，因为更大的多节点 DCP 里分片 attention 的通信会盖过它省的算力；DEP 把请求及其 KV 分到不同 DP rank、同时把 MoE 专家分到各 rank，避开 DCP 的 attention 集合通信。")
...
t(s,"打破队头阻塞。默认 chunked-prefill 按 FIFO，一个长 prefill 会一步步占满整段预算、同 rank 排队的短轮完全排不进去。")
t(s,"对策是 --long-prefill-token-threshold 限制单请求每步最多调度的 token 数；设成 512 时，长 prefill 能让出空间让短轮进同 batch 更早开始 decode。DeepSeek V4 Pro 在 B300 上因此每 GPU 秒 token(TPGS) 最多提升约 93%、p90 交互度约改善 2.3x；代价是长请求自身 TTFT 变高，TTFT 敏感部署应调高阈值。")
...
t(s,"vLLM 用从 300 万美元真实 agentic 编码轨迹(1M 上下文)构建的公开基准、跑在 1000+ 芯片约 2MW 上，展示它是 agentic-first。")
t(s,"对三个开放前沿模型取的是 p90 交互度守住每秒每用户 50 token 高延迟 SLO 下的最高吞吐配置。")
t(s,"DeepSeek V4 Pro：12 芯片 GB300 PD 部署服务 256 个并发 agent 会话、p90 下每人每秒 58.3 token，此工作点每 GPU 秒约 83K 总 token。MiniMax M3：仅 2 张 B300，p90 每秒每人 74.2 token、给到 70K 总 TPGS。Kimi K3：2.8 万亿参数的巨型前沿模型，常规单机装不下；16 张 GB300 在 p90 下每秒每人 62.7 token、处理 11.8K 总 TPGS。")
...
# ---- 图位：按源文顺序显式挂到对应段后（8 张静态图 + 3 张原文 GIF 动图）----
FIG = [
    ...
    (5, 1, "fig07.gif", ""),
    (5, 3, "fig08.gif", ""),
    (7, 0, "fig09.png", "图 9：Kimi K3 在不同硬件上、不同 p90 交互度下每 1 美元能产出的总 token 数。来源：Kimi K3 SemiAnalysis AgentX 仪表盘。"),
    (7, 1, "fig10.png", ""),
    (7, 2, "fig11.png", ""),
]
