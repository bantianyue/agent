...
现代AI工作负载的性能，最终都卡在kernel质量上。编写高性能kernel需要对硬件架构、底层编程语言和优化技术有极其深入的理解，而这类技能在NVIDIA CUDA生态之外极度稀缺。
...
RL进一步强化了SFT中成功的模式：不是发现全新的优化技术，而是让模型学会哪些修改是安全的。成功的RL生成越来越倾向于融合局部操作（激活函数、bias加算），同时保留整体计算结构。结果是在所有三个KernelBench层级上，编译通过率都有显著提升。
