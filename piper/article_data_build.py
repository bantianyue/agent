    ...
    "sections": [
        {
            ...
            "figs": [
                {"src": "fig01.jpg", "caption": "Figure 1：一个用于混合专家模型的 PP x EP x DP 放置。流水线并行把层分布在阶段间、专家并行切分专家 MLP、数据并行复制注意力等非专家组件。"},
            ],
        },
        {
            "type": "h2",
            ...
            "figs": [
                {"src": "fig02.jpg", "caption": "Figure 2：2 路 PP、4 个微批次的 DualPipeV 调度。数字是微批次 ID，加粗单元是重叠的前向-后向微批次对。"},
            ],
        },
        {
            "type": "h3",
            "title": "DP 与 EP 组合时的流调度选择",
            "paras": [
        ...
        },
        {
            ...
            "figs": [
                {"src": "fig04.jpg", "caption": "Figure 4：Piper 架构。用户提供带标注的模型和调度；Piper 将其编译成全局训练 DAG，并用分布式运行时执行每个 worker 的子 DAG。"},
            ],
        },
        {
            "type": "h2",
            ...
            "code": "python examples/test_harness.py --test-file examples/test_qwen.py --base-schedule examples/base-schedules/pp2_dp2_ep2_custom_order.json --schedule custom --ranks 2 --mbs 4 --viz",
        },
        {
            "type": "h3",
            ...
            "code": "PP_TAG = 'PP'\nEP_TAG = 'EP'\n\nclass AnnotatedMoE(MoE):\n    def forward(self, x):\n        ...\n        x = piper.annotate(EP_TAG)(x)  # 包裹专家计算",
        },
        {
            "type": "h3",
            "title": "标注机制与注解的区域",
        ...
        },
        {
            ...
            "title": "逻辑流与点对点通信",
            "paras": [
                "当编译器看到跨设备数据依赖（如前向中阶段 0→1、反向中阶段 1→0）时，会向 IR DAG 加入点对点 send/recv 通信节点，并关联到逻辑流 pp_stream。逻辑流表——GPU 流：一个按序执行操作的工作队列。Piper 用逻辑流标识「彼此应串行、但在依赖和硬件资源允许时可能与其他逻辑流上的操作重叠」的类操作。",
        ...
        {
            "type": "h3",
            "title": "replicate 的可选参数",
            "paras": [
                "`bucket_size` 通过把参数装进 bucket_size-MB 的分组来控制通信粒度——更小的桶可能暴露更多重叠或减少干扰，但也可能降低通信效率。`shard_grads` 对复制区域施加 ZeRO-1 梯度分片。`shard_params` 施加 ZeRO-2 参数分片，这要求在前向/反向算前收集参数分片。`gather_stream` 允许指定用于参数分片相关收集集合通信的独立流，从而更细粒度地控制哪些集合通信被重叠或串行化。",
        ...
        },
        {
            ...
            "title": "用户如何控制重叠与串行化",
            "paras": [
                "逻辑流是用户控制哪些类通信算子可重叠 / 必须串行化的方式：用户不手动协调 CUDA 流，Piper 把逻辑流映射到物理流，仅在数据或时序依赖要求时插入同步。通过通信分桶和流分配，用户可以试验 Figure 3 中那样的多种底层调度策略。",
        ...
        },
        {
            ...
            "code": "{'op': 'split', 'filter': {}, 'dim_name': 'MB', 'num_microbatches': 2}",
        },
        {
            "type": "h3",
            "title": "order 指令与 PASS 标签",
            "paras": [
        ...
        },
        {
            ...
            "figs": [
                {"src": "fig05.png", "caption": "Figure 5：2 路 PP、2 个微批次、带重叠前向-后向微批次对的简单流水线调度。"},
            ],
        },
        {
            "type": "h3",
            ...
            "figs": [
                {"src": "fig06.png", "caption": "Figure 6：重叠前向-后向微批次对的 DAG IR 片段。"},
            ],
            "paras2": [
            ...
            "figs2": [
                {"src": "fig07.jpg", "caption": "Figure 7：重叠前向-后向微批次对的 Profile。EP 和 DP 集合通信被完全隐藏。"},
            ],
        },
        {
            "type": "h3",
        ...
        },
        {
            ...
            "code": "python examples/test_harness.py --test-file examples/test_qwen.py --base-schedule examples/base-schedules/pp4_dp2_ep2_v_placement.json --schedule dualpipev --ranks 2 --mbs 4",
        },
        {
            "type": "h3",
            "title": "运行结果",
        ...
        },
        {
            ...
            "figs": [
                {"src": "fig08.jpg", "caption": "Figure 8：各种流水线调度下 Qwen3 1B 和 Qwen3 9B 的 PP x EP 吞吐。"},
            ],
        },
        {
            "type": "h3",
            "title": "PP x ZeRO",
            "paras": [
                "在 Qwen3 9B 上，TorchTitan 在所评估配置中内存溢出；Piper-DualPipeV 相比 Piper 的 interleaved 1F1B 调度吞吐提升 10%，相比 Megatron 的 interleaved 1F1B 提升 6%。Megatron 不支持 DualPipeV，其 interleaved 调度的最接近基线。除了重叠 EP 通信，我们还计划通过集成 Megatron 的优化内核进一步提升性能。",
            ],
        },
        {
            "type": "h3",
            "title": "PP x ZeRO 内存优势",
            "paras": [
            ...
            "figs": [
                {"src": "fig09.jpg", "caption": "Figure 9：Qwen3 9B 上 PP x ZeRO-2 和 PP x ZeRO-3 的峰值内存。"},
            ],
        },
    ],
