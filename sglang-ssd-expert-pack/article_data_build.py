...
SECTIONS.append({
    ...
    "paras": [
        ...
        B("路由器选中的每个专家都会被执行；"),
        B("Expert Top-K 不变；"),
        B("被选中的专家不会被另一个常驻专家顶替；"),
        B("被选中的专家不会被剪枝、跳过或合并；"),
        B("pack 与清单按配置完成结构、维度和密码学校验；"),
        B("fallback_count 与 io_errors 会被上报，而不是把 I/O 失败悄悄藏起来。"),
...
SECTIONS.append({
    "type": "h3",
    "title": "验证过的权重与 Expert Pack 体积",
    "paras": [
        "验证过的权重文件与生成的 Expert Pack 占用如下：",
        "这些是验证过的权重负载的文件大小。pack 索引、锁、清单和其他元数据另计。",
    ],
    "table": {
        "head": ["模型", "原始权重文件", "权重体积", "Expert Pack 体积"],
        "rows": [
            ["DeepSeek-V4-Flash", "一个 Ollama MXFP4 GGUF blob", "155.10 GB",
             '<strong style="color:#1a7f5a;">147.18 GB</strong>'],
            ["Kimi-K3", "38 个 Q2_K GGUF 分片", "1009.51 GB（约 1.01 TB）",
             '<strong style="color:#1a7f5a;">985.61 GB</strong>'],
        ],
    },
})
...
SECTIONS.append({
    ...
    "fig_after": {"0": [{"src": "fig05.png", "caption": "图5：DeepSeek-V4-Flash 在 Alpaca 与 MMLU 上，SGLang 与基线的预填充、解码 token 速率对比。"}]},
    "table": {
        "head": ["数据集", "基线预填充", "SGLang 预填充", "预填充提升", "基线解码", "SGLang 解码", "解码提升"],
        "rows": [
            ["Alpaca (n=5)", "1.108", '<strong style="color:#1a7f5a;">2.532</strong>',
             '<strong style="color:#1a7f5a;">2.28x</strong>', "0.288",
             '<strong style="color:#1a7f5a;">1.992</strong>', '<strong style="color:#1a7f5a;">6.92x</strong>'],
            ["MMLU (n=5)", "1.223", '<strong style="color:#1a7f5a;">4.141</strong>',
             '<strong style="color:#1a7f5a;">3.39x</strong>', "0.282",
             '<strong style="color:#1a7f5a;">1.846</strong>', '<strong style="color:#1a7f5a;">6.55x</strong>'],
        ],
    },
})
...
SECTIONS.append({
    ...
    "fig_after": {"0": [{"src": "fig06.png", "caption": "图6：Kimi-K3 在 Alpaca 与 MMLU 上，SGLang 与 llama.cpp 的预填充、解码 token 速率对比。"}]},
})

SECTIONS.append({
    "type": "h3",
    "title": "专家缓存命中率与 SSD 流量",
    "paras": [
        ...
        B("Expert Pack 把散落的张量访问变成可寻址的连续专家读取；"),
        B("直接 I/O 去掉了 page cache 到 pinned memory 的拷贝；"),
        B("pinned 中转提供了一块 CUDA 兼容的主机源，让专家级 H2D 不必再经 page cache 中转；"),
        B("GPU 缓存命中时跳过 SSD 读取和 H2D 传输。"),
    ],
    "fig_after": {
        "0": [{"src": "fig07.png", "caption": "图7：DeepSeek-V4-Flash 在 SGLang 上的唯一键显存缓存命中率，以及每生成 token 的平均 SSD 流量。"}],
        "1": [{"src": "fig08.png", "caption": "图8：Kimi-K3 在 SGLang 上的唯一键显存缓存命中率，以及每生成 token 的平均 SSD 流量。"}],
    },
})
...
SECTIONS.append({
    "type": "h3",
    "title": "DeepSeek-V4-Flash：SGLang 与 Ollama",
    "paras": [
        SUB("版本与负载"),
        ...
        CODE("bash", "OLLAMA_HOST=127.0.0.1:11435 ollama serve >/tmp/deepseek-ollama.log 2>&1 &"),
        SUB("准备"),
        ...
        CODE("bash", "ollama pull frob/deepseek-v4-flash-0731\nollama show --modelfile frob/deepseek-v4-flash-0731"),
        ...
        CODE("bash", "python3 examples/runtime/deepseek_v4/benchmark_deepseek_5090.py \\\n  --gguf /path/to/deepseek-v4-flash-0731.gguf \\\n  --validate-only"),
...
SECTIONS.append({
    "type": "h3",
    "title": "Kimi-K3：SGLang 与 llama.cpp",
    "paras": [
        SUB("版本与负载"),
        ...
        CODE("bash", "python3 examples/runtime/kimi_k3/benchmark_kimi_k3_5090.py \\\n  --gguf /path/to/kimi-k3/KIMI-K3-MXP4-DERISKED-Q2_K-00001-of-00038.gguf \\\n  --max-new-tokens 200 --direct-io --read-splits 1 --prepare-only"),
...
SECTIONS.append({
    ...
    "paras": [
        ...
        SUB("缓存与负载"),
        B("GPU 缓存越小，未命中越多，SSD 读取与 H2D 传输就会更频繁地落在关键路径上。"),
        B("提示词或负载分布的变化会改变热点专家，所以某个请求的热集并不保证适配所有负载。"),
        B("如果完整专家池本来就装得进 GPU 显存，Expert Pack 只会多出一条不必要的数据路径，不是合适的部署模式。"),
        B("如果负载由 GPU 计算主导，去掉主机内拷贝带来的收益可能被计算时间掩盖。"),
        B("如果 SSD 的随机读行为、排队或热稳定性不佳，加大 read_splits 和中转槽位可能带来排队与内存压力，而不是吞吐提升。"),
        SUB("当前功能边界"),
...
DATA = {
    ...
    "reference_url": "https://www.lmsys.org/blog/2026-08-29-sglang-ssd-expert-pack/",
    "title": "SGLang SSD Expert Pack：32GB 显存跑起 DeepSeek-V4-Flash 与 Kimi-K3",
}
...
nparas = sum(len(s.get("paras", [])) for s in SECTIONS)
nfigs = sum(len(v) for s in SECTIONS for v in (s.get("fig_after") or {}).values())
print(f"OK {out_path} sections={len(SECTIONS)} paras={nparas} figs={nfigs}")
