...
核心机制是：dLLM会利用顺序灵活性，绕过对探索不可或缺的高不确定性token（如逻辑连接词），优先填容易的部分，导致解覆盖的过早坍缩。这一观察直接动摇了当前dLLM强化学习（RL）方法的设计前提：它们为了保留任意顺序，承担了组合轨迹爆炸、难解似然等巨大复杂度。
...
推理从来不均匀，依赖稀疏的**"分叉token"**（通常是"因此""既然""于是"等连接词）。它们在逻辑轨迹上制造分支，表现为熵的局部尖峰。标准AR解码每步强制解析最左未知token，**迫使模型在分叉点直面不确定性**（图2a），通过采样不同分支保留推理多样性。
