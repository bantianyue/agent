DATA = {
    'title': '从「循环工程」到「图谱工程」：AI 智能体架构正在发生什么',
    'summary': [
        {'key': '一句梗', 'body': 'Peter Steinberger 发了九个字「我们还在聊循环，还是已经切到图谱了？」收获几千赞：整个领域在奔跑中认出了自己。'},
        {'key': '单循环会坏', 'body': '单条改进循环有四种典型失败：古德哈特效应、向上失明、冲突、度量衰减。'},
        {'key': '答案是图谱', 'body': '新兴解法不是更好的循环，而是「循环的图谱」：多个互相看管、约束、纠错的改进周期组成的网络。'},
        {'key': '图谱也翻车', 'body': '没有锚点的图谱会退化成「互相确认的圆」：一切自洽、却从不被验证。拓扑买来复杂，买不来与现实的接触。'},
        {'key': '关键是锚点', 'body': '网络里必须有不可被 argu 的硬测量（真实营收、真实留存），以及来自图外的「什么叫更好」的根本判断。'},
        {'key': ' durable 轴', 'body': '真正分高下的从来不是循环 vs 图谱，而是「有根」vs「无根」：改进机器是否始终贴着它声称要改善的现实。'},
    ],
    'lead': [
    ...
    'sections': [
        ...
        },
        {
            ...
            'paras': [
                ...
                '同样的形状出现在任何把改进做可靠的地方。一家治理得好的公司，是一张以不同速度运转的循环图谱：快的操作循环（每日站会、每周指标）套着慢的管理循环（季度规划），套着更慢的审计循环（年度，而且关键在独立（检查操作循环的数字是否还对应现实），套着最慢的那一圈）董事会问「目标本身还是不是对的目标」。身体也是：温度调节不止一个恒温器，而是一张相互作用的反射网，免疫系统其实就是覆盖整个机体的审计循环，而缓慢的发育过程会重设那些快循环所守护的东西。在每一个例子里，单循环的四种失败，答案都是拓扑式的。',
    ...
    'conclusion': [
        '一句话收束：单循环「变好」、图谱「不变自己骗自己」，而真正分高下的轴是「有根 vs 无根」：你的改进机器是否始终贴着它声称要改善的现实，是否有不可 argu 的硬锚点，是否有来自图外的「什么叫更好」的根本判断。',
        '对做 agent / AI 基础设施的人：别只盯着一个评测循环转。把它扩成图谱（配对反指标、独立审计、速度分层）但别忘了在每个图谱里冻死几条规则、留几个真正碰地的硬测量。否则你只是把「单循环骗自己」升级成了「图谱集体确认、却从不被验证」的更贵版本。',
    
        ' 一个可操作的检验：把你系统里每个「改进循环」列出来，问三个问题：（1）它的指标有没有一个配对的「反指标」在看管它不被博弈？（2）它的参照由谁拥有、多久修订一次？（3）网络里有没有至少一个永远不许被优化的「冻结节点」和一份来自图外的硬测量？三个问题答不出一个，你的图谱就还只是个更好看的无根循环。',
        ' 落到工程上最易被忽略的一点：锚点必须是「没法被 argu」的硬测量。很多团队把「评测分数」当锚点，但评测集一旦被训练/优化循环看见，它就变成了又一个可被博弈的指标：这正是文章里留出集（held-out set）绝不许训练循环看的道理。所以真正的锚点要来自图外：真实营收、真实留存、真实用户访谈，而不是另一份同样由系统生成的报告。',],
    'reference_url': 'https://x.com/IntuitMachine/status/2078419526354378975',
