...
- <strong>SOUL.md 是 Agent 的身份文件</strong>：定义人格、声音、价值观，占据系统提示 1 号槽位<br><br>
- <strong>50 行足够</strong>：200 行消耗 Token 量是 50 行的 5 倍，但效果不会更好<br><br>
- <strong>身份 =/= 指令</strong>：项目指令放 AGENTS.md，工作流放 Skills，事实放 MEMORY.md<br><br>
- <strong>Profile 分发</strong>：一个 SOUL.md = 一个 Agent 人格，可通过 profile install 共享
</div>
</div>
...
**一天中跑多个 Profile 和 Cron 作业时，这 5 倍的差距会迅速累积。**
...
**这会从 `config.yaml` 中加载一个命名人格，叠加在当前会话的 SOUL.md 之上。** 新会话启动时，叠加层消失，SOUL.md 恢复。
...
4. **忽略子 Agent。** Hermes 通过 `delegate_task` 委派任务时，子 Agent 不加载 SOUL.md——它用硬编码的 `DEFAULT_AGENT_IDENTITY`。这是有意设计的：子 Agent 是通用工人。如果需要专门化的子 Agent，请使用独立的 Profile 并通过 Kanban 协调。
...
- **漂移检查（2 周后）：** 启动新会话并重复身份/声音/限制测试。**有深度记忆的 Agent 可能会漂移**，因为积累的上下文压过了身份块。如果发生漂移，灵魂需要更锐利的语言，或者记忆需要修剪。
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
SOUL.md 是 50–80 行文本，定义了你的 Hermes Agent 思考、说话和运作的一切。**它是你设置中杠杆率最高的文件——添加或删除一行就可以改变 Agent 在每一个未来会话中的行为。**<br><br>
有用 Agent 和令人沮丧的 Agent 之间的区别，通常归结为 SOUL.md。不是模型。不是工具。不是提示工程。**是身份。** 从 20 行开始，从经验中迭代，在协作一个月后让 Agent 重写自己的灵魂。<br><br>
最好的灵魂是长出来的，不是设计出来的。
</div>
</div>

---
<span style="font-size:12px;color:#888888;">参考：https://www.hermesbible.com/flows/soul-md-why-50-lines-matter-more-than-your-model</span>
<span style="font-size:12px;color:#888888;">参考：https://hermes-agent.nousresearch.com/docs</span>
