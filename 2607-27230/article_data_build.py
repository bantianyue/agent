    ...
    "sections": [
        {
            ...
            "paras": [
                ...
                "这使深度历史变得可寻址，但检索粒度是"整个宽度"：一个学习 query ∈R^d 给 N=2L+1 个先前来源打分，塌缩成一个关于深度的 softmax 分布 α，于是所有 d 个坐标从同一个深度混合去读。",
            ],
            "fig_after": {
        ...
        },
        {
            ...
            "fig_after": {
                "2": [{"src": "fig02.png", "caption": "Figure 4：H 个路由头确实携带不同的深度-连接：每个头的 token 平均偏差 w̄h−w̄ 相对于头共识的偏离。"}]
            }
        },
        {
            "type": "h2",
        ...
        },
        {
            ...
            "paras": [
                ...
                "**训练速度不受损**：融合路由内核（torch.compiled 参考内核 + 自研 fused routing）在 Table 4/5 里把训练速度和内存开销拉到与标准 Transformer 相当甚至更快，路由操作本身端到端更快。",
            ],
            "fig_after": {
