...
DATA = {
    ...
    "caption_translations": {
        "fig01": "图 1：好的 harness 使得每个单独的 LM 调用都在分布内（locally in-distribution, LID），从而将不熟悉的复杂问题约简为熟悉的简单问题组合。",
        "fig02": "图 2：RLM harness 在不同任务之间诱导出等价关系——主体上下文看到的 token 级轨迹几乎相同，从而实现组合泛化。",
        "fig03": "图 3：RLM 通过上下文卸载和编程式子调用实现了对不同长度任务的高度抽象。",
        "fig04": "图 4：6 个基准上的长度泛化曲线。仅训练短任务，RLM（蓝色）在 8–32 倍长的任务上大幅领先基础 Transformer（黄色），甚至接近 GPT-5.5 RLM。",
        "fig05": "图 5：3 个基准上的策略泛化曲线。RLM（蓝色）在从未训练过的目标领域上仍有可观的提升，基础 Transformer（黄色）几乎为零。",
        "fig06": "图 6：Eval 轨迹与历史训练轨迹之间的相似性分析。RLM 的 root LM 在与训练轨迹的相似度上远超基础 Transformer。",
        "fig07": "图 7：上下文卸载与编程式子调用对比的效果。两者结合相较于仅用其中一种，在泛化性能上有显著提升。",
        "fig08": "图 8：5/6 个长度泛化实验中，RLM 的 eval 轨迹与训练轨迹之间的相似性对比。",
    },
}
