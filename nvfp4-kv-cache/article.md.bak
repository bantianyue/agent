/* 传送门统一样式（add-portal.py 动态注入时引用此 class；真身在内联 style，class 仅兜底） */
...
- 是什么：SGLang、Qwen 与 NVIDIA 把 KV cache 从 FP8 压到 4 bit NVFP4，K/V 存储与读取量约为 FP8 的 56%。- 关键数据：Qwen3.8-27B 同并发 decode 吞吐提升 26% 到 30%，容量驱动下最高提升 78.46%，prefill 几乎不受影响。- 智能体收益：AgentX 上 FP8 KV 在并发 12 后吞吐骤降，NVFP4 KV 靠更高的缓存命中率持续扩展。
...
图 2：SGLang 中的 NVFP4 KV cache 实现
路径 1：初始 prefill
...
NVFP4 KV decode attention kernel
...
图 3：NVFP4 KV attention decode kernel 设计
...
图 8：Qwen3.8-27B 32K/1K ISL/OSL 的 Pareto 曲线
prefill 性能
...
图 9：以 TTFT 衡量的 Qwen3.8-27B prefill 性能
...
图 10：Qwen3.5-397B-A17B 上的 AgentX 性能
输入 token 缓存命中率分析给出了证据：高并发下 FP8 KV 的缓存命中率远低于 NVFP4 KV。

图 11：Qwen3.5-397B-A17B 的 AgentX 输入 token 缓存命中率分析
限制与进行中的工作
...
NVFP4 KV cache 的意义不只是把 KV 压小 44%，而是把省下的显存同时换成两样东西：更快的 decode 和更多常驻请求。带宽受限的长上下文 decode 提速 26% 到 30%，容量驱动下最高 78.46%，这是存储侧优化直接兑换成吞吐的典型样本。对做 LLM serving 的人，AgentX 那组数据更关键：高并发下 FP8 的缓存命中率崩塌、NVFP4 仍在持续扩展，说明 4 bit KV 的真正红利在智能体负载里，显存省下来变成了 prefix 命中率，而不是单次 decode 快的那一点。实验未启用 FP32 全局 scale，校准之后精度差距还有收窄空间，任务级验证仍是上线前的必做项。
