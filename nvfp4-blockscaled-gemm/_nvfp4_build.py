#!/usr/bin/env python3
"""nvfp4 组装：从 _translations 组织 DATA（V1-V7 演进 + 25图 + h3），写 article_data_build.py。"""
import json, os

base = r"D:/06_Hermes/articles/nvfp4-blockscaled-gemm"
...
DATA = {
    "summary": [
        {"key":"优化路径","body":"从基线内核出发，经线程块swizzling、尾声改进、warp专用化存储、消除SFA bank冲突、12路MMA warp、自动调优七轮迭代，在RTX PRO 6000 (SM120) 上逼近cuBLAS。"},
        {"key":"关键收益","body":"对比版本1，版本7在32k问题规模吞吐提升40%，2k提升29%；各规模平均与cuBLAS 13.6相当。"},
        {"key":"核心难点","body":"NVFP4块缩放GEMM的关键在缩放因子的SMEM布局与bank冲突、以及让Tensor Core持续忙碌的warp调度与流水线同步。"},
    ],
    "lead": [
    ...
    "sections": [
        ...
        },
        {
            ...
            "figs":[
                {"src":"fig01.png","caption":"图 2：版本1 的计算吞吐（TFLOP/s）与 cuBLAS 13.5/13.6 对比，8k 处约 1476 TFLOP/s、73% 利用率，32k 下降明显。"},
            ],
        },
        {
            "type":"h2","title":"版本2：线程块 Swizzling",
            "paras":[
            ...
            "figs":[
                {"src":"fig02.png","caption":"图 3：Nsight 内存工作负载分析——DRAM/L2 吞吐与命中率随问题规模变化的指标（决定 swizzle 方向）。"},
                {"src":"fig03.png","caption":"图 4：不同 swizzle_size 选择下单个 wave 加载的 A/B 分块数量示意。"},
                {"src":"fig04.png","caption":"图 5：swizzle_size=16 与默认方案下分块加载情况对比（每 wave 不同分块总数最少为 28）。"},
                {"src":"fig05.png","caption":"图 6：版本2 性能（TFLOP/s）与 cuBLAS 对比，32k 提升最显著（+387 TFLOP/s）。"},
                {"src":"fig06.png","caption":"图 7：版本2 的内存工作负载分析输出，展示 L2/DRAM 指标改善来源。"},
            ],
        },
        {
            "type":"h2","title":"版本3：改进 Epilogue",
            "paras":[
            ...
            "figs":[
                {"src":"fig07.png","caption":"图 8：epilogue 代码路径中 producer_acquire/producer_tail 的同步逻辑示意。"},
                {"src":"fig08.png","caption":"图 9：版本3 性能与 cuBLAS 对比——改进影响很小，处于波动范围。"},
            ],
        },
        {
            "type":"h2","title":"版本4：Warp 专用化存储",
            "paras":[
            ...
            "figs":[
                {"src":"fig09.png","caption":"图 13：warp 专用化存储的流水线结构——存储 warp 与 MMA warp 通过命名屏障协调。"},
                {"src":"fig10.png","caption":"图 12：版本4 性能与 cuBLAS 对比，专用化存储对 Tensor Core 利用率的影响。"},
            ],
        },
        {
            "type":"h2","title":"版本5：消除 SFA bank 冲突",
            "paras":[
            ...
            "figs":[
                {"src":"fig11.png","caption":"图 15：SFA 的 SMEM 布局示意——行 r 与行 r+8 共享 bank 导致冲突的成因图。"},
                {"src":"fig12.png","caption":"图 17：SFA 布局的 bank index 分布可视化，展示冲突来源。"},
                {"src":"fig13.png","caption":"图 18：消除冲突后的 SFA 布局示意图。"},
                {"src":"fig14.png","caption":"图 20：bank 冲突消除前后的访问模式对比。"},
                {"src":"fig15.png","caption":"图 19：版本5 性能与 cuBLAS 对比，bank 冲突消除后的提升。"},
            ],
        },
        {
            "type":"h2","title":"版本6：12 路 MMA warp",
            "paras":[
            ...
            "figs":[
                {"src":"fig16.png","caption":"图 17：版本1-5 与版本6 之间跨波次工作 tile 分配差异。"},
                {"src":"fig17.png","caption":"图 13：版本6 性能与 cuBLAS 对比，更大 tile + 更多 MMA warp 的提升。"},
            ],
        },
        {
            "type":"h2","title":"版本7：自动调优",
            "paras":[
            ...
            "figs":[
                {"src":"fig18.png","caption":"图 23：自动调优的参数空间与搜索过程示意。"},
                {"src":"fig19.png","caption":"图：版本7 性能与 cuBLAS 对比——全规模下的最优结果。"},
            ],
        },
        {
            "type":"h2","title":"性能概览与收尾",
            "paras":[
            ...
            "figs":[
                {"src":"fig20.png","caption":"图：最终结果概览（2k 问题规模）——各版本性能对比。"},
                {"src":"fig21.png","caption":"图：最终结果概览（4k 问题规模）。"},
                {"src":"fig22.png","caption":"图：最终结果概览（8k 问题规模）。"},
                {"src":"fig23.png","caption":"图：最终结果概览（16k 问题规模）。"},
                {"src":"fig24.png","caption":"图：最终结果概览（32k 问题规模）——版本7 较版本1 提升 40%。"},
            ],
        },
        {
            "type":"h2","title":"附录：Cluster Launch Control 与其他缩放因子实验",
            "paras":[
            ...
            "figs":[
                {"src":"fig25.png","caption":"图：CLC 内核性能与 cuBLAS 对比（附录）——与版本7 几乎相同。"},
            ],
        },
    ],
    "conclusion": [
...
print("磁盘fig:", len(disk), "| 引用fig:", len(used))
