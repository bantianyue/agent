...
sec["paras"] += [
    ...
    C(4),
    "接下来要启动 add() kernel，让它在 GPU 上执行。CUDA kernel 的启动使用三尖括号语法 <<< >>>，"
    "加在 add 的调用里、参数列表之前。",
    C(5),
    "就这么简单。尖括号里的内容稍后展开，现在只需要知道这一行启动了一个 GPU 线程来运行 add()。",
    "还有一件事：kernel 启动不会阻塞调用它的 CPU 线程，CPU 必须在读取结果前等 kernel 结束，"
    "所以在最终做误差检查之前要调用 cudaDeviceSynchronize()。",
    "完整代码如下：",
    C(6),
    "CUDA 源代码的文件扩展名是 .cu。把代码存成 add.cu，用 CUDA C++ 编译器 nvcc 编译。",
    C(7, "bash"),
    "这只是第一步。按现在的写法，kernel 只对一个线程是正确的，因为每个运行它的线程都会把整个数组"
...
sec["paras"] += [
    ...
    "一个线程块只能待在一个 SM 上。",
    "以基于 Turing 架构的 NVIDIA T4 GPU 为例，它有 40 个 SM、2560 个 CUDA 核心，每个 SM 最多"
