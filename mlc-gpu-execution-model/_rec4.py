...
TMP=r"D:\06_Hermes\articles\mlc-gpu-execution-model\_frames4"
...
with sync_playwright() as p:
    ...
    keys=pg.evaluate("()=>[...document.querySelectorAll('.p-arrow-label[data-action]')].map(e=>e.dataset.action)")
    print("actions:",keys)
    def shot(times,gap=80):
        global k
    ...
    shot(6)
    for key in keys:
        pg.evaluate("(k)=>{[...document.querySelectorAll('.p-arrow-label[data-action]')].find(e=>e.dataset.action===k).click()}", key)
        pg.wait_for_timeout(450)
        shot(7)
    pg.evaluate("()=>{document.querySelectorAll('.stage-box')[0].click()}")
    pg.wait_for_timeout(450)
    shot(6)
    b.close()
