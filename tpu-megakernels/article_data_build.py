...
字段说明：
  ...
  - title: 公众号推送标题（push-draft.py 直接读这个字段）。**标题口径（2026-09-22）**：
           中英文/数字之间不加空格（「在AMD GPU上跑vLLM」不是「在 AMD GPU 上跑 vLLM」），
           标点一律半角英文标点（：→: ，→, （→( ？→?），英文单词之间的空格保留（AMD GPU）。
           章节标题（sections[].title）同口径；正文/lead/结语仍用中文全角标点。
           preflight-check.py 会 FAIL 并可用 --fix 自动修，但手写时直接写对最省事。
           **来源机构（2026-09-23 用户要求）**：作者/团队来自大厂时，标题带上机构名（如
           「腾讯混元团队:…」「NVIDIA:…」「字节Seed:…」）吸引力明显更强；源文没有明确归属
           就不要硬加，禁止编造。标题禁 emoji/图标前缀。
"""
