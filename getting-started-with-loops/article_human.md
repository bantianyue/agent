...
Loops必须有清晰的边界。设置最大迭代次数、定义明确的停止条件、用成本感知的提示词限制上下文大小。无限循环在Agent世界里的代价不是CPU时间和电量，而是Token消耗。

**怎么开始做这件事？** 从你已经在做的工作里，挑一个你自己是瓶颈的任务。问三个问题：你能写出验证检查吗？目标足够明确吗？工作是否有规律的到达时间？然后运行循环，观察它在哪卡住或过度执行，再迭代改进。关键不是一开始就完美，而是把一次手动操作变成可重复的自动循环。
