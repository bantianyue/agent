...
`SKILL.md` 的片段大概是这样的：告诉AI"不要做完编辑就报告完成，要像人类审阅者那样验证：启动dev server、打开页面、点击交互、检查浏览器控制台零错误"。
...
<div style="font-size:14px;color:#3f3f3f;line-height:1.75;">
这篇文章最核心的价值不是分类本身：按轮次/按目标/按时间/主动算不上什么革命性框架。真正有价值的是那张决策表：你交出了什么、什么时候用哪种模式。很多人在引入Agent循环时犯的错误不是选错了模式，而是根本没意识到自己在手动作循环里。如果你每天花15分钟手动检查PR评论和CI状态，这三个步骤本身就是一个人工版的时间循环：只是缺了一个 `/schedule` 而已。
</div>
</div>
