  ...
  "sections": [
    ...
    },
    {
      ...
      "paras": [
        ...
        "在每轮 投机解码 中（如图 1 所示），一个轻量的 draft 组件提出一个或多个未来 token。这些 token 仅为候选，不会被立即提交。随后 target model 在一次验证 pass 中评估候选 token 序列。",
      ...
      "fig_after": {
        ...
        "9": [
          {
            "src": "fig01.png",
            "caption": "图 1：投机解码的 draft-and-verify 流程：draft 组件提出候选 token，target 模型验证后提交被接受的 token。"
          }
        ]
      }
    },
    {
      ...
      "title": "一个简单的接受/拒绝示例",
      "paras": [
        "图 2 给出了一个投机解码轮次的示例。绿色框是验证通过的 draft token，红色框标记第一个被拒绝的 draft token，灰色框是随后被丢弃的 draft token。输出中的蓝色 token 来自 target model，而非 draft 提议。",
        "假设当前 prompt 为：",
      ...
      "fig_after": {
        "0": [
          {
            "src": "fig02.png",
            "caption": "图 2：一轮投机解码的接受与拒绝：绿框是通过验证的 draft token，红框是第一个被拒 token，灰框是随后被丢弃的 token。"
          }
        ],
        "1": [
          {
    ...
    },
    {
      ...
      "title": "五种方法对比",
      "paras": [
        "图 3 以并排视图直观呈现五种 drafting 方法：draft 组件长什么样、使用哪些 target model 信息，以及候选 token 是串行还是并行生成。图下方的表格以紧凑形式重述同一对比。在这五种方法中，target model 仍以一次 verification pass 评估提议序列，接受判定从左到右依次应用，直到第一个被拒绝的 draft token。"
      ],
      "fig_after": {
        "0": [
          {
            "src": "fig03.png",
            "caption": "图 3：五种起草方法的并排对比：draft 组件结构、使用的 target 模型信息，以及候选 token 的生成方式。"
          }
        ]
      },
      "table": {
